...
const api = {
  server: {
    ...
    updateServerMeta: (id: number, changes: any) =>
      ipcRenderer.invoke('update-server-meta', id, changes),
    ...
    toggleAutoStart: (id: number, enabled: boolean) =>
      ipcRenderer.invoke('toggle-auto-start', id, enabled),
    ...
    writeJson: (id: number, filename: string, data: any) =>
      ipcRenderer.invoke('write-json', id, filename, data),
    getInventory: (id: number, playerName: string) =>
      ipcRenderer.invoke('get-inventory', id, playerName),
    getPlayerNbtStats: (id: number, playerName: string) =>
      ipcRenderer.invoke('get-player-nbt-stats', id, playerName),
    editPlayerNbt: (id: number, playerName: string, stats: any) =>
      ipcRenderer.invoke('edit-player-nbt', id, playerName, stats),
    getSatisfactoryToken: (id: number) => ipcRenderer.invoke('get-satisfactory-token', id),
    saveSatisfactoryToken: (id: number, token: string) =>
      ipcRenderer.invoke('save-satisfactory-token', id, token),
    searchSatisfactoryMods: (query: string, limit?: number, offset?: number) =>
      ipcRenderer.invoke('search-satisfactory-mods', query, limit, offset),
    getInstalledSatisfactoryMods: (serverId: number) =>
      ipcRenderer.invoke('get-installed-satisfactory-mods', serverId),
    installSatisfactoryMod: (serverId: number, modReference: string, downloadLink: string) =>
      ipcRenderer.invoke('install-satisfactory-mod', serverId, modReference, downloadLink),
    uninstallSatisfactoryMod: (serverId: number, modFilename: string) =>
      ipcRenderer.invoke('uninstall-satisfactory-mod', serverId, modFilename),
  ...
  system: {
    ...
    selectCustomBootSound: () => ipcRenderer.invoke('select-custom-boot-sound'),
    getCustomBootSound: () => ipcRenderer.invoke('get-custom-boot-sound'),
    clearCustomBootSound: () => ipcRenderer.invoke('clear-custom-boot-sound'),
    ...
    startTunnel: (ip: string, game: string, port?: number) =>
  ...
  fs: {
    ...
    createFolder: (id: number, relPath: string) =>
      ipcRenderer.invoke('fs-create-folder', id, relPath)
  },
  ...
  backup: {
    ...
    restoreBackup: (id: number, filename: string) =>
      ipcRenderer.invoke('restore-backup', id, filename),
    deleteBackup: (id: number, filename: string) =>
      ipcRenderer.invoke('delete-backup', id, filename),
  ...
  minecraft: {
    ...
    getLoaderVersions: (type: string, mcVersion: string) =>
      ipcRenderer.invoke('get-loader-versions', type, mcVersion),
    downloadServerJar: (id: number, type: string, version: string, loaderVersion?: string) =>
      ipcRenderer.invoke('download-server-jar', id, type, version, loaderVersion),
    changeServerSoftware: (id: number, type: string, version: string, loaderVersion?: string) =>
      ipcRenderer.invoke('change-server-software', id, type, version, loaderVersion),
    searchModpacks: (query: string, version: string, modloader: string) =>
      ipcRenderer.invoke('search-modpacks', query, version, modloader),
    getModpackDetails: (modId: number) => ipcRenderer.invoke('get-modpack-details', modId),
    installCurseforgeModpack: (id: number, modId: number, version: string) =>
      ipcRenderer.invoke('install-curseforge-modpack', id, modId, version),
    getCurseforgeMod: (modId: number) => ipcRenderer.invoke('get-curseforge-mod', modId),
    getCurseforgeFile: (modId: number, fileId: number) =>
      ipcRenderer.invoke('get-curseforge-file', modId, fileId),
    searchCurseforgeMods: (
      ...
      ipcRenderer.invoke('search-curseforge-mods', search, type, version, page, classId, sortField),
    installCurseforgeMod: (id: number, downloadUrl: string, fileName: string, classId?: number) =>
      ipcRenderer.invoke('install-curseforge-mod', id, downloadUrl, fileName, classId),
    getInstalledMods: (id: number, classId?: number) =>
      ipcRenderer.invoke('get-installed-mods', id, classId),
    deleteMod: (id: number, fileName: string, classId?: number) =>
      ipcRenderer.invoke('delete-mod', id, fileName, classId),
    deleteAllMods: (id: number, classId?: number) =>
      ipcRenderer.invoke('delete-all-mods', id, classId)
  },
  ...
  dayz: {
    ...
    updateEconomy: (id: number, settings: any) =>
      ipcRenderer.invoke('update-dayz-economy', id, settings),
    wipeLoot: (id: number, wipePlayers: boolean) =>
      ipcRenderer.invoke('wipe-dayz-loot', id, wipePlayers),
    ...
    installMod: (
      ...
      pass?: string,
      guard?: string
    ) => ipcRenderer.invoke('install-dayz-mod', id, modId, title, user, pass, guard),
    installMods: (
      id: number,
      modsToInstall: { modId: string; modTitle: string }[],
      user?: string,
      pass?: string,
      guard?: string
    ) => ipcRenderer.invoke('install-dayz-mods', id, modsToInstall, user, pass, guard),
    uninstallMod: (id: number, modIdOrFolder: string) =>
      ipcRenderer.invoke('uninstall-dayz-mod', id, modIdOrFolder),
    toggleMapMod: (id: number, folderName: string, isMap: boolean) =>
      ipcRenderer.invoke('toggle-dayz-map-mod', id, folderName, isMap),
    toggleModStatus: (id: number, folderName: string, isDisabled: boolean) =>
      ipcRenderer.invoke('toggle-dayz-mod-status', id, folderName, isDisabled),
    downloadMission: (id: number, modId: string) =>
      ipcRenderer.invoke('download-dayz-mission', id, modId),
    extractLocalMission: (id: number, localMissionsPath: string) =>
      ipcRenderer.invoke('extract-dayz-local-mission', id, localMissionsPath),
  ...
  sevenDaysToDie: {
    ...
    toggleMod: (serverId: number, folderName: string, enabled: boolean) =>
      ipcRenderer.invoke('toggle-7dtd-mod', serverId, folderName, enabled),
    deleteMod: (serverId: number, folderName: string) =>
      ipcRenderer.invoke('delete-7dtd-mod', serverId, folderName),
    setActiveDownloadServer: (serverId: number | null) =>
      ipcRenderer.invoke('set-active-download-server', serverId)
  },

  steam: {
    searchWorkshop: (query: string, queryType?: number, page?: number, requiredTags?: string[]) =>
      ipcRenderer.invoke('search-steam-workshop', query, queryType, page, requiredTags),
    ...
    ) => ipcRenderer.invoke('install-steam-app', id, appId, username, password, steamGuardCode),
    ...
    updateCache: (
      ...
      password?: string,
      steamGuardCode?: string
    ) => ipcRenderer.invoke('update-steam-cache', id, appId, username, password, steamGuardCode),
  ...
  palworld: {
    ...
    setConfig: (serverId: number, config: Record<string, string>) =>
      ipcRenderer.invoke('set-palworld-config', serverId, config),
    searchMods: (query: string, categoryId?: number, index?: number, pageSize?: number) =>
      ipcRenderer.invoke('search-palworld-mods', query, categoryId, index, pageSize),
    getModDetails: (modId: number) => ipcRenderer.invoke('get-palworld-mod', modId),
    installMod: (serverId: number, modId: number, fileId: number) =>
      ipcRenderer.invoke('install-palworld-mod', serverId, modId, fileId),
    getInstalledMods: (serverId: number) =>
      ipcRenderer.invoke('get-installed-palworld-mods', serverId),
    uninstallMod: (serverId: number, modType: string, modName: string) =>
      ipcRenderer.invoke('uninstall-palworld-mod', serverId, modType, modName),
