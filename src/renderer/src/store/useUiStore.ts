...
export const useUiStore = create<UiStore>((set) => ({
  ...
  bootSoundVolume: localStorage.getItem('bootSoundVolume')
    ? Number(localStorage.getItem('bootSoundVolume'))
    : 0.5,

  setActiveGameHub: (hub) =>
    set(() => {
  ...
  setGameCacheStatus: (game, isCached) =>
    set((state) => ({
