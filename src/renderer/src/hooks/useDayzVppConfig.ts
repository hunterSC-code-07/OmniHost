    ...
    try {
      ...
      if (newStatus) {
        if (!serverCfgContent.includes('vppDisablePassword')) {
          ...
          updatedCfg = serverCfgContent.replace(
            /vppDisablePassword\s*=\s*[0-9]+;?/g,
            'vppDisablePassword = 1;'
          )
        }
      } else {
      ...
      if (errorMsg.includes('EBUSY')) {
        alert(
          'Error: The server is currently running and has locked the configuration file. Please shut down your DayZ server before changing this setting.'
        )
      } else {
