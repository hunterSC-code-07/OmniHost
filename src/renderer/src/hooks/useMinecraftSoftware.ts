  ...
  useEffect(() => {
    if (activeTab === 'software') {
      const fetchVersions = async () => {
        let versions: string[] = []
        // @ts-ignore
        if (editingSoftwareType === 'Vanilla')
          versions = await window.api.minecraft.getVanillaVersions()
        // @ts-ignore
        else if (editingSoftwareType === 'Paper')
          versions = await window.api.minecraft.getPaperVersions()
        // @ts-ignore
        else if (editingSoftwareType === 'Fabric')
          versions = await window.api.minecraft.getFabricVersions()
        // @ts-ignore
        else if (editingSoftwareType === 'Forge')
          versions = await window.api.minecraft.getForgeVersions()
        // @ts-ignore
        else if (editingSoftwareType === 'NeoForge')
  ...
  useEffect(() => {
    if (
      ...
      const fetchLoaderVersions = async () => {
        // @ts-ignore
        const loaders = await window.api.minecraft.getLoaderVersions(
          editingSoftwareType,
          editingSoftwareVersion
        )
