...
export const useDayzInstalledMods = () => {
  ...
  const { installedMods, installedModsLoading, loadInstalledMods, setInstalledModsLoading } =
    useDayzModStore()
  ...
  return {
    ...
    loadInstalledMods: async () => {
      if (activeServerId) await loadInstalledMods(activeServerId)
    },
    setLoading: (loading: boolean) => {
      if (activeServerId) setInstalledModsLoading(activeServerId, loading)
    },
    activeServerId
  }
}
