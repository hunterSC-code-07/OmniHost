  ...
  const handleSaveConfig = async () => {
    if (activeServerId !== null) {
      ...
      try {
        ...
        if (props['gamemode']) {
          // @ts-ignore
          await window.api.server.sendCommand(activeServerId, `defaultgamemode ${props['gamemode']}`);
        }
        if (props['spawn-monsters'] !== undefined) {
          const enableMonsters = props['spawn-monsters'] === 'true';
          // @ts-ignore
          await window.api.server.sendCommand(activeServerId, `gamerule doMobSpawning ${enableMonsters}`);
          // @ts-ignore
          await window.api.server.sendCommand(activeServerId, `gamerule doPatrolSpawning ${enableMonsters}`);
        }
        if (props['spawn-npcs'] !== undefined) {
          const enableNpcs = props['spawn-npcs'] === 'true';
          // @ts-ignore
          await window.api.server.sendCommand(activeServerId, `gamerule doTraderSpawning ${enableNpcs}`);
        }
        if (props['white-list'] !== undefined) {
          // @ts-ignore
          await window.api.server.sendCommand(activeServerId, `whitelist ${props['white-list'] === 'true' ? 'on' : 'off'}`);
        }
      } catch (e) {}
