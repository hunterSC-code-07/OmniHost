...
export function useMinecraftMods(
  ...
  const [installAllProgress, setInstallAllProgress] = useState<{
    current: number
    total: number
    text: string
  ...
  const handleInstallMissingDependency = async (depId: string) => {
    ...
    try {
      ...
      if (results && results.length > 0) {
        const bestMatch =
          results.find(
            (m: any) => m.slug === depId || m.name.toLowerCase() === depId.toLowerCase()
          ) || results[0]
  ...
  const handleInstallAllMissingDependencies = async () => {
    ...
    const missingIds = Array.from(
      new Set(
        modDependencies.flatMap((m) =>
          (m.dependencies || []).filter((d: any) => !d.satisfied).map((d: any) => d.id)
        )
      )
    )
    ...
    setInstallAllProgress({
      ...
      text: `Starting installation of ${missingIds.length} dependencies...`
    })
  ...
  const handleInstallModpack = async (modpack: any) => {
    ...
    // @ts-ignore
    const cleanup = window.api.server.onDownloadProgress(
      activeServerId,
    ...
    try {
      // @ts-ignore
      await window.api.minecraft.installCurseforgeModpack(
        activeServerId,
        modpack.id,
        serverMeta.version
      )
  ...
  const handleDeleteAllMods = async () => {
    if (activeServerId === null) return
    if (
      !confirm('Are you sure you want to delete ALL installed mods, shaders, and resource packs?')
    )
      return
    // @ts-ignore
