...
export const SoftwareTab: React.FC<SoftwareTabProps> = React.memo(() => {
  ...
  const {
    editingSoftwareType,
    setEditingSoftwareType,
    editingSoftwareVersion,
    setEditingSoftwareVersion,
    editingLoaderVersion,
    setEditingLoaderVersion,
    editingAvailableVersions,
    editingAvailableLoaderVersions,
    isTypeMenuOpen,
    setIsTypeMenuOpen,
    isVersionMenuOpen,
          ...
          <div className="bg-[#0a0a0a] border border-[#4CAF50]/30 rounded-xl max-w-md w-full shadow-[0_0_40px_rgba(76,175,80,0.1)] overflow-hidden">
            <div className="p-6">
              ...
              <p className="text-gray-300 leading-relaxed text-sm">
                Changing an existing server to a CurseForge modpack is not recommended as it may
                cause severe compatibility issues or corruption.
                <br />
                <br />
                It is highly recommended to create a new server for the modpack instead. Do you want
                to go to the Create Server screen now?
              </p>
            </div>
              ...
              <div className="flex-1 flex flex-col items-center justify-center">
                ...
                {downloadProgress > 0 && (
                  <div className="w-full max-w-md bg-gray-800 rounded-full h-2 mt-4 overflow-hidden">
                    <div
                      className="bg-[#4CAF50] h-2 transition-all duration-300"
              ...
              <div className="flex flex-col gap-6 max-w-xl">
                      ...
                      >
                        {editingAvailableVersions.map((opt) => (
                          <div
                            key={opt}
                            onClick={() => {
                              setEditingSoftwareVersion(opt)
                              setIsVersionMenuOpen(false)
                            }}
                            className={`px-4 py-2.5 cursor-pointer hover:bg-white/10 transition-colors ${editingSoftwareVersion === opt ? 'text-brand font-bold' : 'text-[#bfbfbf]'}`}
                          >
                            {opt}{' '}
                            {editingSoftwareVersion === opt && (
                              <span className="float-right text-brand">✓</span>
                            )}
                          </div>
                        ))}
                ...
                {['Forge', 'Fabric', 'NeoForge'].includes(editingSoftwareType) && (
                  <div className="relative z-30">
                    ...
                    <button
                      onClick={() => {
                        if (!isChangingSoftware && editingAvailableLoaderVersions.length > 0)
                          setIsLoaderMenuOpen(!isLoaderMenuOpen)
                      }}
                    ...
                    >
                      {editingAvailableLoaderVersions.length === 0
                        ? 'Loading...'
                        : editingLoaderVersion || 'Select version'}
                      <svg
                        ...
                        >
                          {editingAvailableLoaderVersions.map((opt) => (
                            <div
                              key={opt}
                              onClick={() => {
                                setEditingLoaderVersion(opt)
                                setIsLoaderMenuOpen(false)
                              }}
                              className={`px-4 py-2.5 cursor-pointer hover:bg-white/10 transition-colors ${editingLoaderVersion === opt ? 'text-brand font-bold' : 'text-[#bfbfbf]'}`}
                            >
                              {opt}{' '}
                              {editingLoaderVersion === opt && (
                                <span className="float-right text-brand">✓</span>
                              )}
                            </div>
                          ))}
                ...
                <div className="bg-yellow-900/20 border border-yellow-700/50 rounded-lg p-4 mt-4">
                  <p className="text-sm text-yellow-500 font-bold mb-1">
                    Warning: Mod Compatibility
                  </p>
                  <p className="text-xs text-yellow-600">
                    Changing software versions or types may cause compatibility issues with
                    installed mods. Old mods will be moved to a backup folder.
                  </p>
                </div>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      // @ts-ignore
                      window.api.server.onDownloadProgress(
                        activeServerId,
