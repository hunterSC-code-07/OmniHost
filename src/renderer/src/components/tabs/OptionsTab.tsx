...
export const OptionsTab: React.FC<OptionsTabProps> = React.memo(({ serverId, onConfigSaved }) => {
  const {
    rawConfigText,
    setRawConfigText,
    advancedMode,
  ...
  const ConfigToggle = ({ label, propKey, invert = false }: any) => {
    const rawVal =
      (props[propKey] !== undefined ? props[propKey] : DEFAULT_PROPS[propKey]) === 'true'
      ...
      <div className="bg-black/5 backdrop-blur-sm border border-surface-container-highest hover:border-brand/40 transition-colors flex justify-between items-center px-5 py-3 w-full h-14 shadow-sm rounded-xl">
        ...
        <button
          onClick={() =>
            setProps((prev) => ({
              ...prev,
              [propKey]: invert ? (isToggled ? 'true' : 'false') : !isToggled ? 'true' : 'false'
            }))
          }
        ...
        >
          <div
            className={`absolute top-1 w-4 h-4 rounded-full transition-transform duration-300 shadow-md ${isToggled ? 'translate-x-7 bg-background' : 'translate-x-1 bg-on-surface-variant'}`}
          ></div>
        </button>
      </div>
    )
              ...
              <div className="absolute top-full right-0 mt-2 bg-[#0a0a0a]/95 backdrop-blur-3xl border border-white/20 rounded-md shadow-[0_8px_32px_rgba(0,0,0,0.8)] z-50 py-2 w-48 max-h-60 overflow-y-auto">
                {options.map((opt: string) => (
                  <div
                    key={opt}
                    onClick={() => {
                      setProps((prev) => ({ ...prev, [propKey]: opt }))
                      setIsOpen(false)
                    }}
      ...
      <div className="bg-black/5 backdrop-blur-sm border border-surface-container-highest hover:border-brand/40 transition-colors flex justify-between items-center px-5 py-3 w-full h-14 shadow-sm rounded-xl">
        ...
        <div className="flex items-center gap-1 bg-background border border-surface-container-highest rounded-lg overflow-hidden focus-within:border-brand focus-within:ring-1 focus-within:ring-brand/50 transition-all">
          <button
            type="button"
            onClick={dec}
            className="px-3 py-1.5 hover:bg-surface-container-highest hover:text-brand transition-colors text-on-surface-variant font-bold border-r border-surface-container-highest"
          >
            -
          </button>
          <input
          ...
          <button
            type="button"
            onClick={inc}
            className="px-3 py-1.5 hover:bg-surface-container-highest hover:text-brand transition-colors text-on-surface-variant font-bold border-l border-surface-container-highest"
          >
            +
          </button>
        </div>
      ...
      >
        <div className="p-6 bg-transparent font-body flex flex-col gap-6 min-h-full">
          ...
          <div className="flex justify-between items-end">
            <div>
              <h2 className="font-headline-lg text-headline-lg text-brand drop-shadow-[0_0_8px_rgba(255,215,0,0.3)]">
                Options
              </h2>
            </div>
              ...
              <div className="bg-transparent p-6 border border-surface-container-highest rounded-2xl shadow-inner">
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                  ...
                  <ConfigString
                    ...
                    propKey="resource-pack"
                    placeholder="https://example.com/pack.zip"
                    isFullWidth={true}
                  />
                  <ConfigString
                    label="Resource pack prompt"
                    propKey="resource-pack-prompt"
                    placeholder="Please accept this resource pack"
                    isFullWidth={true}
                  />
                </div>
              </div>
