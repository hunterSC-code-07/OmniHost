...
export const ConsoleTab: React.FC<ConsoleTabProps> = React.memo(({ onPlayerClick, isActive }) => {
  ...
  const activeLogs = activeServerId
    ? logs.filter((l) => l.id === activeServerId.toString() || l.id === 'global')
    : []
    ...
    <div className="absolute inset-0 flex gap-6 p-6 min-h-0 bg-transparent font-body">
      <div className="flex-1 flex flex-col bg-black/40 backdrop-blur-md rounded-xl overflow-hidden border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.3)] min-h-0 min-w-0">
        ...
        >
          ...
          <button
            type="submit"
            className="bg-brand/10 hover:bg-brand/20 text-brand border border-brand/50 hover:border-brand shadow-[0_0_15px_rgba(255,215,0,0.1)] px-8 py-2.5 rounded-xl font-bold transition-all uppercase tracking-widest text-sm"
          >
            Send
          </button>
        </form>
                    ...
                    <img
                      ...
                      onError={(e) => {
                        ;(e.target as HTMLImageElement).src =
                          `https://minotar.net/helm/${playerName}/32.png`
                      }}
                    />
