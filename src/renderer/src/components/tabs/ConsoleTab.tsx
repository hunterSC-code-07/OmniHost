...
export const ConsoleTab: React.FC<ConsoleTabProps> = React.memo(({ onPlayerClick, isActive }) => {
  ...
  const activeLogs = activeServerId
    ? logs.filter((l) => l.id === activeServerId.toString() || l.id === 'global').map((l) => l.msg)
    : []
  ...
  return (
    <div className="absolute inset-0 flex gap-6 p-6 min-h-0 bg-transparent font-body">
      <div
        className={`flex-1 flex flex-col overflow-hidden min-h-0 min-w-0 ${isMinecraft ? 'bg-black/5 backdrop-blur-sm border-[2px] border-white/10 text-white' : isPalworld ? 'pal-panel-dark' : isTerraria ? 'terraria-panel-dark' : 'bg-black/40 backdrop-blur-md rounded-xl border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.3)]'}`}
      >
        <div
          ref={parentRef}
        ...
        >
          <div
            className="font-mono text-sm text-on-surface-variant shadow-inner w-full relative"
            style={{ height: `${rowVirtualizer.getTotalSize()}px` }}
          >
            {activeLogs.length === 0 && (
              <div className="text-on-surface-variant/50 italic mt-4 mb-4">
                Waiting for server output... click Start to boot!
              </div>
            )}
            {rowVirtualizer.getVirtualItems().map((virtualItem) => {
              ...
              return (
                ...
                >
                  {log.includes('INFO') ? (
                    <span className="text-yellow-400 font-bold">INFO </span>
                  ) : (
                    ''
                  )}
                  {log.includes('WARN') ? (
                  ...
                  <span
                    className={
                      log.includes('joined the game')
                        ? 'text-green-400 font-bold'
                        : log.includes('left the game')
                          ? 'text-gray-500'
                          : log.startsWith('>')
        ...
        >
          ...
          <button
            ...
            className={
              isMinecraft
                ? 'minecraft-btn minecraft-btn-red flex items-center justify-center h-[42px] px-3'
                : isPalworld
                  ? 'pal-btn flex items-center justify-center h-[42px] px-3 text-red-400'
                  : isTerraria
                    ? 'terraria-btn terraria-btn-red flex items-center justify-center h-[42px] px-3'
                    : 'p-2.5 text-on-surface-variant hover:text-red-400 bg-surface-container-highest/50 hover:bg-red-500/10 rounded-lg transition-all border border-transparent hover:border-red-500/30 flex items-center justify-center group'
            }
            title="Clear Console"
          >
          ...
          <button
            ...
            className={
              isMinecraft
                ? 'minecraft-btn minecraft-btn-green w-32 h-[42px] ml-2'
                : isPalworld
                  ? 'pal-btn pal-btn-blue w-32 h-[42px] ml-2'
                  : isTerraria
                    ? 'terraria-btn terraria-btn-green w-32 h-[42px] ml-2'
                    : 'bg-brand/10 hover:bg-brand/20 text-brand border border-brand/50 hover:border-brand shadow-[0_0_15px_rgba(255,215,0,0.1)] px-8 py-2.5 rounded-xl font-bold transition-all uppercase tracking-widest text-sm'
            }
          >
            Send
          </button>
      ...
      <div
        className={`w-72 flex flex-col min-h-0 overflow-hidden ${isMinecraft ? 'bg-black/5 backdrop-blur-sm border-[2px] border-white/10 text-white' : isPalworld ? 'pal-panel-dark' : isTerraria ? 'terraria-panel-dark' : 'bg-black/20 backdrop-blur-md border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.2)] rounded-xl'}`}
      >
              ...
              <div className="space-y-3">
                {activePlayers.map((playerName, idx) => (
                  <div
                    ...
                    className={
                      isMinecraft
                        ? 'flex items-center gap-4 cursor-pointer group minecraft-btn text-left mb-2 w-full p-2'
                        : isPalworld
                          ? 'flex items-center gap-4 cursor-pointer group pal-btn border-none hover:bg-white/10 text-left mb-2 w-full p-2'
                          : isTerraria
                            ? 'flex items-center gap-4 cursor-pointer group terraria-btn text-left mb-2 w-full p-2'
                            : 'flex items-center gap-4 bg-surface-container-lowest p-3.5 rounded-xl border border-surface-container-highest shadow-sm cursor-pointer hover:border-brand/50 hover:bg-surface-container-lowest/80 transition-colors group'
                    }
                  >
