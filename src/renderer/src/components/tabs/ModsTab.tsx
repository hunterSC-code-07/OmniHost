      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 mb-6 shrink-0">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            {serverMeta
              ? `Server Type: ${serverMeta.type} ${serverMeta.version}`
              : 'Loading server info...'}
          </p>
        </div>
        ...
        <>
          ...
          {modViewType === 'browse' && (
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
              ...
              <div className="flex flex-col md:flex-row justify-between items-center bg-black/40 backdrop-blur-md p-3 rounded-lg border border-white/5 mb-4 text-[#bfbfbf] text-sm shrink-0 shadow-inner relative z-50">
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 hover:text-white px-3 py-1 font-bold"
                  >
                ...
                <div className="flex items-center gap-4">
                  <div className="relative">
                    <button
                      ...
                      className="flex items-center gap-2 hover:text-white px-3 py-1 font-bold bg-transparent text-[#bfbfbf]"
                    >
                      {[
                        { id: 1, name: 'Sort: Featured' },
                        { id: 2, name: 'Sort: Popularity' },
                        { id: 3, name: 'Sort: Last Updated' },
                        { id: 4, name: 'Sort: Name' }
                      ].find((o) => o.id === activeSortField)?.name || 'Sort: Popularity'}
                      <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" />
                      </svg>
                    </button>
                      ...
                      <div className="absolute top-full right-0 mt-2 w-48 bg-[#0a0a0a]/95 backdrop-blur-3xl border border-white/20 rounded-md shadow-[0_8px_32px_rgba(0,0,0,0.8)] z-50 py-2">
                        {[
                          ...
                          <div
                            key={opt.id}
                            onClick={() => {
                              setActiveSortField(opt.id)
                              setIsSortMenuOpen(false)
                            }}
                            className={`px-4 py-2.5 cursor-pointer hover:bg-white/10 transition-colors ${activeSortField === opt.id ? 'text-brand font-bold' : 'text-[#bfbfbf]'}`}
                          >
                            {opt.name}{' '}
                            {activeSortField === opt.id && (
                              <span className="float-right text-brand">✓</span>
                            )}
                          </div>
                        ))}
              ...
              <form
                onSubmit={(e) => handleSearchMods(e)}
                className="mb-6 flex gap-3 shrink-0 relative z-40"
              >
                <input
                  type="text"
                    ...
                    {modResults.map((mod: any) => {
                      const isInstalled = installedMods.some((m) =>
                        m.name
                          .toLowerCase()
                          .includes(
                            mod.slug?.replace(/-/g, '') || mod.name.toLowerCase().replace(/ /g, '')
                          )
                      )
                      return (
                        <React.Fragment key={mod.id}>
                          <div className="bg-black/30 backdrop-blur-sm p-4 flex gap-4 group transition-colors hover:bg-black/50">
                            <img
                              ...
                              alt={mod.name}
                              className="w-[84px] h-[84px] rounded shadow-md bg-black/50 object-cover flex-shrink-0"
                            />

                            <div className="flex-1 min-w-0 flex flex-col justify-between">
                              <div>
                                <div className="flex items-center gap-2 mb-1">
                                  <h4 className="font-bold text-[#e0e0e0] text-lg truncate group-hover:text-white transition-colors">
                                    {mod.name}
                                  </h4>
                                  <span className="text-sm text-[#888888]">
                                    by{' '}
                                    <span className="text-[#cccccc]">{mod.authors?.[0]?.name}</span>
                                  </span>
                                </div>
                                <p className="text-sm text-[#aaaaaa] line-clamp-1 mb-3">
                                  {mod.summary}
                                </p>
                              </div>
                              ...
                              <div className="flex items-center justify-between mt-auto w-full">
                                <div className="flex flex-wrap items-center gap-1.5">
                                  <span className="text-[11px] bg-[#333333] text-[#cccccc] px-2 py-0.5 rounded border border-[#444444] font-semibold">
                                    {classOptions.find((c) => c.id === activeClassId)?.name ||
                                      'Mods'}
                                  </span>
                                  {mod.categories?.slice(0, 3).map((cat: any) => (
                                    <span
                                      key={cat.id}
                                      className="text-[11px] text-[#aaaaaa] px-1 font-semibold"
                                    >
                                      {cat.name}
                                    </span>
                                  ))}
                                ...
                                <div className="flex items-center gap-4 text-xs text-[#888888] shrink-0 font-semibold">
                                  <span className="flex items-center gap-1.5">
                                    <svg
                                      ...
                                      viewBox="0 0 20 20"
                                    >
                                      <path d="M10 2a8 8 0 100 16 8 8 0 000-16zM9.5 14h1v1h-1v-1zm1-8h-1v6h1V6z" />
                                    </svg>{' '}
                                  ...
                                  {mod.latestFiles?.[0]?.fileLength && (
                                    <span className="flex items-center gap-1.5">
                                      ...
                                      {formatBytes(mod.latestFiles[0].fileLength)}
                                    </span>
                                  )}
                                  {serverMeta && (
                                    <span className="flex items-center gap-1.5 text-[#aaaaaa]">
                                      ...
                                      >
                                        <path
                                          ...
                                          strokeLinejoin="round"
                                          strokeWidth="2"
                                          d="M11 4a2 2 0 114 0v1a1 1 0 001 1h3a1 1 0 011 1v3a1 1 0 01-1 1h-1a2 2 0 100 4h1a1 1 0 011 1v3a1 1 0 01-1 1h-3a1 1 0 01-1-1v-1a2 2 0 10-4 0v1a1 1 0 01-1 1H7a1 1 0 01-1-1v-3a1 1 0 00-1-1H4a2 2 0 110-4h1a1 1 0 001-1V7a1 1 0 00-1-1H4a1 1 0 01-1-1V4a1 1 0 011-1h3a1 1 0 001-1v-1z"
                                        ></path>
                                      </svg>{' '}
                            ...
                            <div className="flex items-center justify-center ml-4 shrink-0">
                              <button
                                ...
                                disabled={installingModId !== null || isInstalled}
                                className={`px-5 py-2 rounded-md font-bold transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-[0_4px_14px_rgba(0,0,0,0.5)] ${isInstalled ? 'bg-black/40 text-gray-400 border border-white/5' : 'bg-brand hover:brightness-110 text-black shadow-[0_0_15px_rgba(76,175,80,0.3)]'}`}
                              >
                                {isInstalled
                                  ? 'Installed'
                ...
                {installedMods.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-gray-500 text-sm mt-1 max-w-sm">
                      Browse mods in the Browse tab or install a modpack to get started.
                    </p>
                  </div>
                ) : (
                            ...
                            >
                              ...
                              <td className="px-6 py-4 text-right">
                                <button
                                  ...
                                  className="text-red-400 hover:text-white hover:bg-red-500/80 px-3 py-1.5 rounded text-xs font-bold transition-all flex items-center gap-1.5 ml-auto"
                                >
          ...
          {modViewType === 'dependencies' && (
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center gap-3 mt-0.5">
                      <h4
                        className={`text-2xl font-bold ${depStats.missingDeps > 0 ? 'text-amber-400' : 'text-gray-400'}`}
                      >
                  ...
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center ${depStats.missingDeps > 0 ? 'bg-amber-500/10 text-amber-400' : 'bg-white/5 text-gray-400'}`}
                  >
              ...
              <div className="flex items-center justify-between gap-3 mb-4 shrink-0">
                ...
                >
                  <RefreshCw
                    className={`w-4 h-4 ${isLoadingDependencies ? 'animate-spin text-brand' : ''}`}
                  />
                  Refresh
                </button>
              </div>
                                    ...
                                    >
                                      ...
                                      <div className="flex items-center gap-1.5">
                                        ...
                                        {dep.version && dep.version !== '*' && (
                                          <span className="text-[11px] font-mono text-gray-400 opacity-80">
                                            ({dep.version})
                                          </span>
                                        )}
                                        <span
          ...
          {modViewType === 'modpacks' && (
            <div className="flex-1 flex flex-col min-h-0 animate-in fade-in duration-300">
              {/* Modpacks Search Bar */}
              <form
                onSubmit={(e) => handleSearchModpacks(e)}
                className="mb-6 flex gap-3 shrink-0 relative z-40"
              >
                <input
                  type="text"
                ...
                <div className="bg-brand/10 border border-brand/30 rounded-xl p-4 mb-4 flex items-center gap-3 animate-pulse text-brand shrink-0">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-brand/80">
                      {modpackProgressText || 'Downloading archive and extracting server mods...'}
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                {isSearchingModpacks ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-gray-500 text-sm mt-1 max-w-sm">
                      Try searching for popular modpacks like Fabulously Optimized or Cobblemon.
                    </p>
                  </div>
                ) : (
                            ...
                            <div className="flex flex-wrap items-center justify-between gap-3 mt-auto">
                              <div className="flex flex-wrap items-center gap-1.5">
                                ...
                                {pack.categories?.slice(0, 4).map((c: any) => (
                                  <span
                                    key={c.id}
                                    className="text-xs bg-white/5 text-gray-300 px-2 py-0.5 rounded border border-white/5"
                                  >
                                    {c.name}
                                  </span>
                                ))}
                              ...
                              <div className="flex items-center gap-4 text-xs text-gray-400 font-semibold">
                                {pack.downloadCount && (
                                  <span className="flex items-center gap-1">
                                    ...
                                    {(pack.downloadCount / 1000000).toFixed(1)}M
                                  </span>
                                )}
                                {serverMeta && (
              ...
              {installingModId !== null && (
                <div className="bg-brand/10 border border-brand/30 rounded-xl p-4 mb-4 flex items-center gap-3 animate-pulse text-brand shrink-0">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-brand/80">
                      {installProgressText || 'Downloading and installing shaderpack...'}
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                {isSearchingShaders ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-gray-500 text-sm mt-1 max-w-sm">
                      Try searching for popular shaders like Complementary, BSL, or MakeUp.
                    </p>
                  </div>
                ) : (
                    ...
                    {shaderResults.map((shader: any) => {
                      ...
                      return (
                        ...
                        >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {shader.downloadCount && (
                                <p className="text-xs text-gray-400 flex items-center gap-1">
                                  <Download className="w-3 h-3" />{' '}
                                  {(shader.downloadCount / 1000000).toFixed(1)}M
                                </p>
                              )}
                            </div>
                          </div>
                          ...
                          <p
                            className="text-sm text-gray-400 line-clamp-2 mb-4 flex-1"
                            dangerouslySetInnerHTML={{ __html: shader.summary || '' }}
                          ></p>
              ...
              {installingModId !== null && (
                <div className="bg-brand/10 border border-brand/30 rounded-xl p-4 mb-4 flex items-center gap-3 animate-pulse text-brand shrink-0">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-brand/80">
                      {installProgressText || 'Downloading and installing resource pack...'}
                    </p>
                  </div>
                </div>
              )}
              ...
              >
                {isSearchingResourcePacks ? (
                  ...
                  <div className="flex flex-col items-center justify-center py-20 text-center">
                    ...
                    <p className="text-gray-500 text-sm mt-1 max-w-sm">
                      Try searching for popular resource packs like Faithful, Sphax PureBDcraft, or
                      Bare Bones.
                    </p>
                  </div>
                ) : (
                    ...
                    {resourcePackResults.map((pack: any) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <p
                            className="text-sm text-gray-400 line-clamp-2 mb-4 flex-1"
                            dangerouslySetInnerHTML={{ __html: pack.summary || '' }}
                          ></p>
