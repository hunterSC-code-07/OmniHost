  ...
  palworld: {
    ...
    fs: `
      ...
      void main() {
          ...
          vec3 deepBlue = vec3(0.005, 0.02, 0.05);
          vec3 brightBlue = vec3(0.05, 0.3, 0.6);
          vec3 gold = vec3(0.9, 0.7, 0.1);

          float combinedNoise = mix(n1, n2, n3 * 0.4);
          vec3 color = mix(deepBlue, brightBlue, combinedNoise);
          
          float highlightMask = pow(n1 * n2, 4.0);
          color = mix(color, gold, highlightMask * 0.6);
