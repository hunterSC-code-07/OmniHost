...
export const TopNavbar: React.FC = () => {
  ...
  const [cacheToClear, setCacheToClear] = useState<{
    id: string
    name: string
    warning: string
  } | null>(null)
  ...
  const cacheOptions = [
    {
      ...
      size: cacheSizes.minecraft,
      warning:
        'This will delete downloaded server jars and CurseForge mods. They will be re-downloaded automatically when needed.'
    },
    {
      id: 'dayzBase',
      name: 'DayZ Server Files',
      size: cacheSizes.dayzBase,
      warning:
        'This deletes the cached DayZ dedicated server files. New servers will require a full download from SteamCMD.'
    },
    {
      id: 'dayzWorkshop',
      name: 'DayZ Workshop Mods',
      size: cacheSizes.dayzWorkshop,
      warning:
        'This deletes the downloaded DayZ workshop mods cache. SteamCMD will be forced to redownload all mods next time you update a server.'
    },
    {
      id: 'satisfactoryBase',
      name: 'Satisfactory Server Files',
      size: cacheSizes.satisfactoryBase,
      warning:
        'This deletes the cached Satisfactory dedicated server files. New servers will require a full download from SteamCMD.'
    }
  ]
      ...
      <div className="h-full px-gutter w-full flex items-center justify-between">
        <div className="flex items-center gap-8">
          ...
          <nav className="hidden md:flex items-center gap-6">
            <button
              ...
              className={`flex items-center gap-2 font-bold transition-all [-webkit-app-region:no-drag] ${activeServerId === null && activeGameHub === null ? 'text-primary' : 'text-on-surface-variant hover:text-on-surface'}`}
            >
        ...
        <div className="flex items-center gap-6 mr-[150px]">
          <button
            ...
            className="relative overflow-hidden group px-2.5 py-2 rounded-lg border bg-surface/40 border-outline-variant/30 text-on-surface-variant hover:text-primary hover:border-primary/50 transition-all flex items-center justify-center [-webkit-app-region:no-drag]"
          >
              ...
              {isCacheMenuOpen && (
                <>
                  <div
                    ...
                    onClick={() => setIsCacheMenuOpen(false)}
                  ></div>
            ...
            <div className="p-6">
              ...
              <p className="text-gray-400 leading-relaxed text-sm">
                ...
                <span className="text-white font-bold">
                  {formatBytes(cacheOptions.find((o) => o.id === cacheToClear.id)?.size || 0)}
                </span>{' '}
                of cached data.
                <br />
                <br />
