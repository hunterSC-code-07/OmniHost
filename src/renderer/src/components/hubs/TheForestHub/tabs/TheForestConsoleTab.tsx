        ...
        <div className="flex-1 flex flex-col bg-[var(--forest-gray-dark)] border border-white/10 min-h-0 min-w-0">
          ...
          >
            <div className="p-6 font-mono text-white/80 font-bold flex flex-col min-h-full text-sm">
              {logs.length === 0 && (
                <div className="text-white/50 italic mt-4 mb-4">
                  Waiting for The Forest server output... click Start to boot!
                </div>
              )}
              {logs.map((log, i) => {
                ...
                return (
                  ...
                  >
                    {isCommand && (
                      <span className="text-[var(--forest-green)] font-bold mr-1">&gt;</span>
                    )}
                    <span>{cleanLog.replace(/^> /, '').trim()}</span>
                  </div>
                )
              })}
          ...
          <form onSubmit={onSubmit} className="p-4 bg-[var(--forest-gray)] flex gap-3 items-center">
            <span className="text-[var(--forest-yellow)] font-bold text-xl leading-none flex items-center">
              &gt;
            </span>
            <input
              type="text"
              ...
              className="forest-input flex-1 !font-mono !border-none !text-left pl-2"
            />

            <button
              type="button"
              onClick={handleClearLogs}
              className="p-2.5 text-white/50 hover:text-[var(--forest-red)] transition-all flex items-center justify-center group"
              title="Clear Console"
            >
            ...
            <button
              type="submit"
              className="forest-btn text-[var(--forest-yellow)] hover:text-white"
            >
              SEND
            </button>
          </form>
      ...
      <div className="w-80 flex flex-col min-h-0">
        ...
        <div className="flex-1 flex flex-col bg-[var(--forest-gray-dark)] border border-white/10 min-h-0 min-w-0">
          <div className="p-5 border-b border-white/10 flex justify-between items-center bg-[var(--forest-gray)]">
            ...
            <div className="font-bold text-sm text-[var(--forest-yellow)]">
          ...
          >
            <div className="p-4 flex flex-col min-h-full">
              {onlinePlayers.length === 0 ? (
                <div className="text-center text-white/30 font-bold mt-10 uppercase">
                  No one is online.
                </div>
              ) : (
                <div className="space-y-2">
                  {onlinePlayers.map((playerName, idx) => (
                    <div
                      ...
                      className="flex items-center gap-4 bg-[var(--forest-gray)] p-3 cursor-pointer hover:bg-white/10 transition-all group"
                    >
