...
const SettingSlider: React.FC<SettingSliderProps> = ({ label, value, onChange, min, max, step, note }) => (
  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-3 border-b border-white/5 last:border-0 hover:bg-white/[0.02] px-4 -mx-4 rounded-lg transition-colors group">
    ...
    <div className="flex items-center gap-4 flex-1 max-w-[300px]">
      <span className="font-mono text-sm text-brand w-12 text-right">{value}</span>
      <input 
        type="range" 
        min={min} 
        max={max} 
        ...
        className="w-full h-1.5 bg-surface-container-highest rounded-full appearance-none cursor-pointer accent-brand shadow-inner group-hover:bg-surface-container-highest/80 transition-colors"
      />
    </div>
  </div>
);
  ...
  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 py-3 border-b border-white/5 last:border-0 hover:bg-white/[0.02] px-4 -mx-4 rounded-lg transition-colors group">
    ...
    <div className="flex items-center gap-4 flex-1 max-w-[300px] justify-end">
      <select 
        value={value}
        onChange={(e) => onChange(e.target.value)}
        className="bg-surface-container border border-outline-variant/30 text-on-surface text-sm rounded-lg focus:ring-brand focus:border-brand block w-full p-2"
      >
        {options.map((opt) => (
...
export const PalworldOptionsTab: React.FC<PalworldOptionsTabProps> = React.memo(({
  serverId
}) => {
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="bg-surface-container-low border border-surface-container-highest rounded-2xl p-8 shadow-glass flex flex-col items-center">
          <div className="text-center mb-6">
            <h3 className="font-headline-lg text-headline-lg text-on-surface">{cpuLimit} shared CPU cores</h3>
            <p className="font-label-lg text-label-lg text-on-surface-variant uppercase tracking-widest mt-2">Palworld automatically manages RAM usage (Up to ~32GB recommended)</p>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="bg-surface-container-low border border-surface-container-highest hover:border-brand/40 transition-colors flex justify-between items-center px-6 py-4 shadow-sm rounded-xl h-16">
            <span className="font-label-md text-label-md text-on-surface uppercase tracking-widest">Stop when empty for 15 mins</span>
            <button 
        ...
        {Object.keys(gameSettings).length > 0 && (
          <div className="bg-surface-container-low border border-surface-container-highest rounded-2xl p-8 shadow-glass">
            <h3 className="font-headline-sm text-headline-sm text-brand mb-6 uppercase tracking-widest">Game Settings</h3>
            
            <div className="flex flex-col gap-1">
              <SettingSlider label="Day Time Speed" value={getFloat('DayTimeSpeedRate')} onChange={(v: string) => updateGameSetting('DayTimeSpeedRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Night Time Speed" value={getFloat('NightTimeSpeedRate')} onChange={(v: string) => updateGameSetting('NightTimeSpeedRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="EXP Rate" value={getFloat('ExpRate')} onChange={(v: string) => updateGameSetting('ExpRate', Number(v).toFixed(6))} min={0} max={20} step={0.1} />
              <SettingSlider label="Pal Capture Rate" value={getFloat('PalCaptureRate')} onChange={(v: string) => updateGameSetting('PalCaptureRate', Number(v).toFixed(6))} min={0.1} max={2} step={0.1} />
              <SettingSlider label="Pal Appearance Rate" value={getFloat('PalSpawnNumRate')} onChange={(v: string) => updateGameSetting('PalSpawnNumRate', Number(v).toFixed(6))} min={0.1} max={3} step={0.1} note="*Note: Affects game performance" />
              <SettingSlider label="Damage from Pals Multiplier" value={getFloat('PalDamageRateAttack')} onChange={(v: string) => updateGameSetting('PalDamageRateAttack', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Damage to Pals Multiplier" value={getFloat('PalDamageRateDefense')} onChange={(v: string) => updateGameSetting('PalDamageRateDefense', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Pal Hunger Depletion Rate" value={getFloat('PalStomachDecreaceRate')} onChange={(v: string) => updateGameSetting('PalStomachDecreaceRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Pal Stamina Reduction Rate" value={getFloat('PalStaminaDecreaceRate')} onChange={(v: string) => updateGameSetting('PalStaminaDecreaceRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Pal Auto Health Regeneration Rate" value={getFloat('PalAutoHPRegeneRate')} onChange={(v: string) => updateGameSetting('PalAutoHPRegeneRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Damage from Player Multiplier" value={getFloat('PlayerDamageRateAttack')} onChange={(v: string) => updateGameSetting('PlayerDamageRateAttack', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Damage to Player Multiplier" value={getFloat('PlayerDamageRateDefense')} onChange={(v: string) => updateGameSetting('PlayerDamageRateDefense', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Player Hunger Depletion Rate" value={getFloat('PlayerStomachDecreaceRate')} onChange={(v: string) => updateGameSetting('PlayerStomachDecreaceRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Player Stamina Reduction Rate" value={getFloat('PlayerStaminaDecreaceRate')} onChange={(v: string) => updateGameSetting('PlayerStaminaDecreaceRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              <SettingSlider label="Player Auto Health Regeneration Rate" value={getFloat('PlayerAutoHPRegeneRate')} onChange={(v: string) => updateGameSetting('PlayerAutoHPRegeneRate', Number(v).toFixed(6))} min={0.1} max={5} step={0.1} />
              
              <div className="my-4 border-t border-white/5"></div>
              
              <SettingSlider label="Maximum Number of Dropped Items in a World" value={getInt('DropItemMaxNum', 3000)} onChange={(v: string) => updateGameSetting('DropItemMaxNum', v)} min={0} max={5000} step={100} note="Increasing the limit may affect processing load." />
              <SettingSlider label="Maximum Number of Guild Members" value={getInt('GuildPlayerMaxNum', 20)} onChange={(v: string) => updateGameSetting('GuildPlayerMaxNum', v)} min={1} max={100} step={1} />
              <SettingSlider label="Maximum number of bases for each guild" value={getInt('BaseCampMaxNumInGuild', 4)} onChange={(v: string) => updateGameSetting('BaseCampMaxNumInGuild', v)} min={1} max={10} step={1} />
              <SettingSlider label="Maximum number of work pals at the base" value={getInt('BaseCampWorkerMaxNum', 15)} onChange={(v: string) => updateGameSetting('BaseCampWorkerMaxNum', v)} min={1} max={20} step={1} />
