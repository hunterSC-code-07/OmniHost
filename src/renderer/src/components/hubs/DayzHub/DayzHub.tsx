        ...
        <div className="flex justify-between items-center relative z-20">
          ...
          <div className="flex gap-3 items-center">
            <div className="flex glass-panel rounded-lg overflow-hidden transition-all hover:border-white/30">
              ...
              <button className="px-3 border-l border-white/10 opacity-50 cursor-not-allowed text-gray-600 flex items-center justify-center" title="No settings for Radmin VPN">
                <span className="material-symbols-outlined text-[18px] leading-none">settings</span>
              </button>
            </div>
            <button onClick={() => handleDelete(activeServerId)} className="relative overflow-hidden group glass-panel px-6 py-2.5 rounded-lg font-bold transition-all hover:border-red-500/60 hover:shadow-[0_8px_32px_rgba(248,113,113,0.2),inset_0_1px_2px_rgba(255,255,255,0.4)] text-red-400 hover:text-red-300">
            ...
            <button onClick={() => activeServer.status === 'Online' ? handleStop(activeServerId) : handleStart(activeServerId)} className={`relative overflow-hidden group glass-panel px-8 py-2.5 rounded-lg font-bold transition-all ${activeServer.status === 'Online' ? 'hover:border-red-500/60 text-red-400 hover:text-red-300' : 'hover:border-green-500/60 text-green-400 hover:text-green-300'}`}>
              <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-white/20 opacity-30 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none"></div>
              <span className="relative z-10">{activeServer.status === 'Online' ? 'STOP' : 'START'}</span>
            </button>
            <button onClick={() => handleRestart(activeServerId)} className="relative overflow-hidden group glass-panel px-8 py-2.5 rounded-lg font-bold transition-all hover:border-brand/60 text-brand hover:text-green-300">
              <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-white/20 opacity-30 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none"></div>
