...
export const DayzModsTab: React.FC<DayzModsTabProps> = ({ onNavigateToInstalled }) => {
  ...
  const { downloadProgress, installingMod, setInstallingMod, handleInstall, handleUninstall } =
    useDayzModInstallation(activeServerId, installedMods, onNavigateToInstalled, loadInstalledMods)
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`pb-3 text-sm font-bold transition-colors relative ${
                activeCategory === cat.id ? 'text-red-400' : 'text-gray-400 hover:text-white'
              }`}
            >
              {cat.label}
      ...
      {showCreds && (
        <div className="p-6 bg-black/60 backdrop-blur-xl border border-red-500/30 shadow-[0_0_30px_rgba(220,38,38,0.15)] m-6 rounded-xl flex flex-col gap-4">
          ...
          <p className="text-gray-400 text-sm">
            DayZ Workshop mods require a Steam account that owns the game. Your credentials are used
            locally by SteamCMD.
          </p>
          <div className="flex gap-3 items-center flex-wrap mt-2">
            <input
              ...
              className="bg-black/40 border border-white/10 rounded-lg px-4 py-2.5 text-sm flex-1 min-w-[200px] text-white outline-none focus:border-red-500/50"
            />
            <input
              type="password"
            ...
            <input
              type="text"
              placeholder="Steam Guard (if needed)"
              value={steamCreds.steamGuard}
              onChange={(e) => setSteamCreds({ ...steamCreds, steamGuard: e.target.value })}
              className="bg-black/40 border border-white/10 rounded-lg px-4 py-2.5 text-sm w-48 text-white outline-none focus:border-red-500/50"
            />
      ...
      <div className="flex-1 min-h-0 overflow-hidden flex flex-col">
        {activeCategory === 99 ? (
          <OverlayScrollbarsComponent
            ...
            className="flex-1 custom-scrollbar p-6"
          >
            <div className="bg-black/60 backdrop-blur-xl border border-red-500/30 rounded-xl p-8 flex flex-col gap-6 shadow-[0_0_30px_rgba(220,38,38,0.15)] max-w-4xl mx-auto mt-8">
              <div>
                ...
                <p className="text-gray-400 text-sm mt-2">
                  Select your DayZ game folder to automatically find and import your client mods.
                  This is useful if you are moving a server or already have mods downloaded via the
                  DayZ Launcher.
                </p>
              </div>
              <div className="flex items-center gap-3">
            ...
            >
              <div className="p-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {results.map((mod) => {
                  ...
                  const progress = activeServerId
                    ? pendingDownloads[activeServerId]?.[mod.id] || downloadProgress[mod.id]
                    : downloadProgress[mod.id]

                  return (
                    <div
                      key={mod.id}
                      className="bg-black/30 backdrop-blur-sm border border-white/5 hover:border-red-500/30 hover:bg-black/50 transition-colors shadow-lg rounded-xl p-4 flex gap-4 group"
                    >
                      {mod.thumbnail ? (
                        <img
                          src={mod.thumbnail}
                          alt={mod.title}
                          className="w-20 h-20 object-cover rounded-lg bg-black/50 shadow-md group-hover:scale-105 transition-transform shrink-0"
                        />
                      ) : (
                      ...
                      <div className="flex-1 flex flex-col justify-between overflow-hidden min-w-0">
                        <div>
                          <h3
                            className="text-sm font-bold text-white truncate group-hover:text-red-300 transition-colors"
                            title={mod.title}
                          >
                            {mod.title}
                          </h3>
                          <p className="text-xs text-gray-400 line-clamp-2 mt-1">
                            {stripBBCode(mod.description) || 'No description'}
                          </p>
                        </div>
                          ...
                          {progress ? (
                            <div className="w-full mt-auto">
                              <div className="flex justify-between items-center mb-1">
                                <div className="text-[10px] text-red-400 font-bold truncate">
                                ...
                                <button
                                  onClick={(e) => {
                                    ...
                                    if (removePendingDownload && activeServerId)
                                      removePendingDownload(activeServerId, mod.id)
                                  }}
                              ...
                              <div className="h-1 w-full bg-black/60 rounded-full overflow-hidden border border-white/5 relative">
                                <div
                                  ...
                                  style={{
                                    width: `${(progress as any)?.progress ?? (progress as any)?.percent ?? 0}%`
                                  }}
                                ></div>
                              </div>
                            </div>
                ...
                {hasMore && results.length > 0 && (
                  <div
                    ref={loadMoreRef}
                    className="col-span-full py-6 flex justify-center items-center"
                  >
                    {loading && (
            ...
            >
              <div className="flex flex-col md:flex-row gap-6 items-start">
                {viewingMod.thumbnail ? (
                  <img
                    src={viewingMod.thumbnail}
                    alt={viewingMod.title}
                    className="w-full md:w-64 md:h-64 object-cover rounded-lg bg-black/50 shadow-md shrink-0 border border-white/5"
                  />
                ) : (
                ...
                <div className="flex flex-col gap-4 flex-1 min-w-0">
                  ...
                  <div className="text-sm text-gray-300 whitespace-pre-wrap break-words overflow-wrap-anywhere leading-relaxed mt-2 bg-black/20 p-4 rounded-lg border border-white/5 font-mono">
                    {stripBBCode(viewingMod.description) ||
