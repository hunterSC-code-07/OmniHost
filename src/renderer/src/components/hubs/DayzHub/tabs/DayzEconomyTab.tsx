          ...
          <div className="flex justify-between items-end">
            <div>
              ...
              <p className="text-sm text-gray-400">
                Tweak loot spawns and item conditions. These changes overwrite your mission's{' '}
                <code className="text-red-400">globals.xml</code> and{' '}
                <code className="text-red-400">types.xml</code>.
              </p>
              <div className="flex items-center gap-2 mt-3">
                <span className="text-sm font-bold text-gray-300">Targeting Map / Mission:</span>
                <span className="text-sm font-mono bg-red-900/30 text-red-400 border border-red-500/30 px-2 py-1 rounded">
                  {template}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between p-4 bg-[#121212] rounded-lg border border-white/5">
              <div>
                ...
                <p className="text-sm text-gray-400 mt-1">
                  Forces all naturally spawned items on the map to be in pristine condition by
                  setting LootDamageMax to 0.
                </p>
              </div>
              <button
              ...
              >
                <div
                  className={`absolute top-1 left-1 bg-white w-4 h-4 rounded-full transition-transform ${pristineLoot ? 'translate-x-6' : 'translate-x-0'}`}
                />
              </button>
            </div>
            ...
            <div>
              ...
              <div className="mt-8 bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Adjusting these sliders will multiply the{' '}
                    <code className="text-blue-300">nominal</code> and{' '}
                    <code className="text-blue-300">min</code> tags for every item in that category
                    inside <code className="text-blue-300">types.xml</code>. Backups are
                    automatically created before any edits are made.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between p-4 bg-[#121212] rounded-lg border border-red-500/20">
                <div>
                  ...
                  <p className="text-sm text-gray-400 mt-1">
                    Deletes the storage files on the map ({template}). On the next server restart,
                    the economy will generate fresh loot across the entire map using your new
                    settings. You will have the option to keep or wipe player character data.
                  </p>
                </div>
                <button
                  onClick={async () => {
                    if (
                      confirm(
                        `Are you sure you want to wipe all spawned items on ${template}? This will delete the storage files and cannot be undone.`
                      )
                    ) {
                      const wipePlayers = confirm(
                        'Do you want to wipe player data as well? Click OK to wipe player data, or Cancel to preserve player characters.'
                      )
                      try {
                        // @ts-ignore
                        ...
                        if (success) {
                          alert(
                            `Loot storage successfully wiped!${wipePlayers ? ' Player data was also wiped.' : ' Player data was preserved.'} Restart the server to generate fresh loot.`
                          )
                        } else {
                          alert(
                            'Failed to wipe loot. Storage file may not exist yet or an error occurred.'
                          )
                        }
                      } catch (e: any) {
                        if (e.message?.includes('SERVER_IS_RUNNING')) {
                          alert(
                            'Error: You must shut down the server before wiping the loot! If you wipe the loot while the server is running, the server will just overwrite the wipe with its memory state when it shuts down.'
                          )
                        } else {
                          alert(
                            'Failed to wipe loot. Storage file may not exist yet or an error occurred.'
