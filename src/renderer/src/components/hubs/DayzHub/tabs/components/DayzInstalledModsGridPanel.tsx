...
export const DayzInstalledModsGridPanel: React.FC<DayzInstalledModsGridPanelProps> = ({
  ...
  const { downloadingMission, handleDownloadMission, handleExtractLocalMission } =
  ...
  const { handleUninstall } = useDayzModUninstall(
    activeServerId,
    mods,
    loadInstalledMods,
    setLoading
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {activeServerId &&
            ...
            Object.values(pendingDownloads[activeServerId]).map((pending: any) => {
