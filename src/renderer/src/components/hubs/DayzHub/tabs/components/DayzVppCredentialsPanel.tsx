...
export const DayzVppCredentialsPanel: React.FC<DayzVppCredentialsPanelProps> = ({
  activeServerId
}) => {
  ...
  } = useDayzVppCredentials(activeServerId)
    ...
    <div className="bg-black/30 backdrop-blur-sm rounded-xl border border-white/5 shadow-lg overflow-hidden flex flex-col">
      ...
      <div className="p-6">
        <p className="text-sm text-gray-400 mb-4">
          Manage server login passwords for VPP Admin Tools.
        </p>
        ...
        <div className="mt-4 flex items-center justify-between bg-black/20 p-4 rounded-lg border border-white/5">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-1">
              Disable the password requirement completely. Only players listed as Super Admins will
              be able to access the tools.
            </p>
          </div>

          <div className="flex items-center gap-3">
            <span
              className={`text-[10px] font-bold ${!disablePassword ? 'text-gray-400' : 'text-primary'}`}
            >
            ...
            >
              <div
                className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${!disablePassword ? 'translate-x-0' : 'translate-x-5'}`}
              />
            </div>
          </div>
        </div>
