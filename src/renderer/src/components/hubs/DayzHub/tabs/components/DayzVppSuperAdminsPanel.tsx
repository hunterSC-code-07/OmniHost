...
export const DayzVppSuperAdminsPanel: React.FC<DayzVppSuperAdminsPanelProps> = ({
  activeServerId
}) => {
  const { superAdmins, newAdmin, setNewAdmin, handleAddAdmin, handleRemoveAdmin } =
    useDayzVppAdmins(activeServerId)
    ...
    <div className="bg-black/30 backdrop-blur-sm rounded-xl border border-white/5 shadow-lg overflow-hidden flex flex-col">
      ...
      <div className="p-6">
        <p className="text-sm text-gray-400 mb-4">
          Manage the list of Steam64 IDs with SuperAdmin privileges.
        </p>
        ...
        <div className="bg-surface-container-highest border border-white/10 rounded-lg overflow-hidden">
          ...
          {superAdmins.map((admin, idx) => (
            <div
              key={idx}
              className="flex justify-between items-center p-3 border-b border-white/5 hover:bg-white/5 transition-colors group"
            >
