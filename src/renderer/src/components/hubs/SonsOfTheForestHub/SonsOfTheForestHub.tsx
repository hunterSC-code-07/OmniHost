...
export const SonsOfTheForestHub: React.FC = () => {
  ...
  return (
    <div
      className="gamehub-theme flex-1 flex flex-col relative overflow-hidden sotf-scrollbars sotf-ui"
      data-game="sons-of-the-forest"
    >
      <div
      ...
      <div className="hub-frame-header flex flex-col z-10 relative px-10 pt-10 pb-4">
        <div className="hub-header-row flex justify-between items-center w-full mb-8">
          <button
            ...
            className="sotf-btn flex items-center justify-center hover:-translate-x-1"
            title="BACK"
          >
          ...
          <div className="hub-header-actions flex gap-6 items-center">
            <div className="flex bg-[var(--sotf-panel)] items-center border border-[var(--sotf-border)]">
              <button
                ...
                className={`px-4 py-2 flex items-center justify-center transition-colors ${tunnelStatus === 'Online' ? 'text-green-500' : tunnelStatus === 'Starting...' ? 'text-gray-500 cursor-not-allowed' : 'text-gray-400 hover:text-white'}`}
              >
                <span
              ...
              <button
                ...
                className="px-3 py-2 border-l border-[var(--sotf-border)] text-gray-400 hover:text-white transition-colors flex items-center justify-center"
      ...
      <div className="hub-frame-content flex-1 overflow-hidden relative min-h-0 flex flex-col z-10 px-10 pb-10">
        ...
        <div className="flex justify-between items-center w-full mt-6 pt-4 border-t border-[var(--sotf-border)]">
          <button className="sotf-btn hover:-translate-x-1 opacity-0 pointer-events-none">
            BACK
          </button>
          <div className="flex gap-8">
            <button
              onClick={() => deleteServer(activeServer.id)}
              className="sotf-btn hover:text-red-500"
            >
              DELETE
            </button>
            <button
              ...
              className={`sotf-btn ${activeServer.status === 'Online' ? 'hover:text-red-500' : 'hover:text-green-500'}`}
            >
              {activeServer.status === 'Online' ? 'STOP' : 'START'}
            </button>
            <button
              onClick={() => restartServer(activeServer.id)}
              className="sotf-btn hover:text-blue-400"
            >
              RESTART
            </button>
          </div>
