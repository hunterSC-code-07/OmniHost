    ...
    <div className="absolute inset-0 flex flex-col p-4 min-h-0 bg-transparent gap-4">
      ...
      <div className="flex-1 flex flex-col bg-[var(--sotf-panel)] border border-[var(--sotf-border)] overflow-hidden min-h-0 p-8 items-center justify-center">
        <div className="flex flex-col items-center gap-6">
          <div
            className={`w-32 h-32 rounded-full border-4 flex items-center justify-center ${activeServer.status === 'Online' ? 'border-[var(--sotf-highlight)]' : 'border-[var(--sotf-border)]'}`}
          >
            <span
              className={`material-symbols-outlined text-[64px] ${activeServer.status === 'Online' ? 'text-[var(--sotf-highlight)]' : 'text-[var(--sotf-text-dim)]'}`}
            >
          ...
          <div className="text-center">
            <h2 className="text-4xl sotf-glitch-text-strong uppercase mb-2">
              {activeServer.status}
            </h2>
        ...
        <div className="w-full max-w-2xl mt-16 grid grid-cols-2 gap-8 border-t border-[var(--sotf-border)] pt-8">
          <div className="flex flex-col items-center">
            <span className="text-[var(--sotf-text-dim)] font-bold uppercase tracking-widest text-sm mb-2">
              MAX PLAYERS
            </span>
