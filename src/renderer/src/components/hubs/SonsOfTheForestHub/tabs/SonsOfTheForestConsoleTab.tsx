      ...
      <div className="flex-1 flex flex-col bg-[var(--sotf-panel)] border border-[var(--sotf-border)] overflow-hidden min-h-0">
        ...
        >
          <div className="p-4 font-mono text-sm text-[var(--sotf-text-dim)] flex flex-col min-h-full">
            {logs.length === 0 && (
              <div className="italic mt-4 mb-4 text-center">WAITING FOR SERVER OUTPUT...</div>
            )}
            {logs.map((log, i) => {
        ...
        <form
          onSubmit={onSubmit}
          className="p-4 bg-transparent border-t border-[var(--sotf-border)] flex gap-3 items-center"
        >
          <span className="text-[var(--sotf-text-dim)] font-bold text-xl leading-none flex items-center">
            &gt;
          </span>
          <input
            type="text"
          ...
          <button
            ...
            className="sotf-btn text-sm hover:text-red-500"
            title="Clear Console"
          >
            CLEAR
          </button>
          <button type="submit" className="sotf-btn text-sm text-[var(--sotf-highlight)]">
            SEND
          </button>
        </form>
      </div>
