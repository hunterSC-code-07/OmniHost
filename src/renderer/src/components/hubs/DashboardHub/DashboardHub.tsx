...
export function DashboardHub({ getGameImageUrl, isGameSupported }: any) {
  ...
  const handleSteamCacheUpdate = async (): Promise<void> => {
    ...
    const hubConfig = HUB_REGISTRY[activeGameHub]
    if (!hubConfig?.steamAppId) return
    if (!hubConfig.steamAnonymous) {
      openSteamLoginModal('cache')
      return
    }

    setUpdatingSteamCacheGame(activeGameHub)
    try {
      await window.api.steam.updateCache(0, hubConfig.steamAppId)
                      ...
                      {activeGameHub && HUB_REGISTRY[activeGameHub]?.steamAppId && (
                        <div className="flex flex-col md:flex-row items-center gap-4 mt-4 bg-surface-container p-6 rounded-xl border border-outline-variant/30">
                          ...
                          <div className="flex flex-wrap gap-3 items-center ml-auto">
                            {!gameCacheStatus[activeGameHub] ? (
                              <button
                                onClick={() => void handleSteamCacheUpdate()}
                                disabled={updatingSteamCacheGame === activeGameHub}
                              ...
                              >
                                <span
                                  className={`material-symbols-outlined text-[18px] ${updatingSteamCacheGame === activeGameHub ? 'animate-spin' : ''}`}
                                >
                                  {updatingSteamCacheGame === activeGameHub ? 'sync' : 'download'}
                                </span>
                                {updatingSteamCacheGame === activeGameHub
                                  ? 'Downloading...'
                                  : 'Download Base'}
                              </button>
                            ) : (
                              <>
                                ...
                                <button
                                  onClick={async () => {
                                    if (
                                      confirm(
                                        `Are you sure you want to delete the cached ${activeGameHub} base files?`
                                      )
                                    ) {
                                      // @ts-ignore
                                      await window.api.steam.deleteCache(
                                        HUB_REGISTRY[activeGameHub].steamAppId
                                      )
