...
export const TerrariaOverviewTab: React.FC = () => {
  ...
  const currentStat =
    serverStats.length > 0 ? serverStats[serverStats.length - 1] : { cpu: 0, ram: 0 }
    ...
    <div className="p-8 flex flex-col gap-6 max-w-7xl mx-auto w-full pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 items-stretch">
        <div className="terraria-panel p-6 flex flex-col h-[220px] relative overflow-hidden group">
          ...
          <div className="flex justify-between items-center h-8 z-10">
            ...
            <span className="text-2xl text-green-400 font-black drop-shadow-[0_0_8px_rgba(74,222,128,0.5)]">
              {currentStat.cpu.toFixed(1)}%
            </span>
          </div>
          <div className="h-32 w-full relative overflow-hidden rounded-lg bg-black/60 border border-white/5 mt-auto z-10">
            ...
            <div className="absolute bottom-1 right-2 text-xs text-gray-500 font-bold uppercase tracking-widest">
              60s History
            </div>
          </div>
        </div>
        ...
        <div className="terraria-panel p-6 flex flex-col h-[220px] relative overflow-hidden group">
          ...
          <div className="flex justify-between items-center h-8 z-10">
            ...
            <span className="text-2xl text-blue-400 font-black drop-shadow-[0_0_8px_rgba(96,165,250,0.5)]">
              {(currentStat.ram / 1024 / 1024).toFixed(1)} MB{' '}
              <span className="text-sm text-gray-500 font-bold">/ {maxRam.toFixed(0)}GB</span>
            </span>
          </div>
          <div className="h-32 w-full relative overflow-hidden rounded-lg bg-black/60 border border-white/5 mt-auto z-10">
            <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="terraria-panel p-6 flex items-center gap-4">
          <div
            className={`w-12 h-12 rounded-lg flex items-center justify-center ${serverStatus === 'Online' ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}
          >
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <div
                className={`w-2.5 h-2.5 rounded-full ${serverStatus === 'Online' ? 'bg-green-400 shadow-[0_0_10px_#4ade80] animate-pulse' : 'bg-red-500 shadow-[0_0_10px_#ef4444]'}`}
              ></div>
