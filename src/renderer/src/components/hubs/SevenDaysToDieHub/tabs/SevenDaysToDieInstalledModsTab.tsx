  ...
  return (
    <div className="flex-1 flex flex-col p-8 overflow-hidden sevendays-ui min-h-0">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={fetchMods} className="sevendays-btn px-6 py-2 flex items-center gap-2">
        ...
        >
          {loading ? (
            ...
            <div className="p-12 flex flex-col items-center justify-center text-center h-full">
              <span className="material-symbols-outlined text-6xl text-[var(--7dtd-text-dim)] mb-4">
                extension_off
              </span>
              <h3 className="sevendays-title text-2xl mb-2">NO MODS INSTALLED</h3>
              <p className="text-[var(--7dtd-text-dim)] uppercase">
                Download mods from the Nexus Mods or Community Mods tabs.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {mods.map((mod) => (
                <div
                  key={mod.folderName}
                  className={`sevendays-panel flex flex-col p-5 border border-[var(--7dtd-border)] ${mod.enabled ? 'border-l-4 border-l-white bg-[var(--7dtd-bg-panel-light)]' : 'border-l-4 border-l-white/20 opacity-70'}`}
                >
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <h3
                        className="sevendays-title text-xl leading-tight truncate pr-2"
                        title={mod.name}
                      >
                        {mod.name}
                      </h3>
                      <div className="text-sm text-white/50 font-bold mt-1 uppercase">
                        v{mod.version}
                      </div>
                    </div>
                  ...
                  <p className="text-sm text-[var(--7dtd-text-dim)] line-clamp-2 uppercase">
                  ...
                  <div className="mt-auto pt-4 border-t border-[var(--7dtd-border)] flex justify-between items-center text-xs text-[var(--7dtd-text-dim)] uppercase font-bold">
                    <span className="truncate" title={mod.folderName}>
                      {mod.folderName}
                    </span>
