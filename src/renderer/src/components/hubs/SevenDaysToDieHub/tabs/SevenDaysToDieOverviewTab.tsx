...
export const SevenDaysToDieOverviewTab: React.FC<SevenDaysToDieOverviewTabProps> = React.memo(
  ({ serverId }) => {
          ...
          <div className="p-8 flex flex-col gap-6 w-full">
              ...
              <div className="sevendays-panel flex flex-col h-[220px] relative overflow-hidden group p-6 border border-[var(--7dtd-border)] hover:border-white/50 transition-colors">
                <div className="flex justify-between items-center h-8 z-10 mb-4">
                  <h3 className="sevendays-title text-xl flex items-center gap-2">
                    <span className="material-symbols-outlined text-[var(--7dtd-text-dim)]">
                      memory
                    </span>
                    CPU LOAD
                  </h3>
                ...
                <div className="flex-1 relative overflow-hidden bg-[var(--7dtd-bg-panel-dark)] border border-[var(--7dtd-border)] mt-auto z-10">
                  ...
                  <div className="absolute bottom-1 right-2 text-xs text-white/50 font-bold uppercase tracking-widest">
                    60S HISTORY
                  </div>
                </div>
              </div>
              ...
              <div className="sevendays-panel flex flex-col h-[220px] relative overflow-hidden group p-6 border border-[var(--7dtd-border)] hover:border-white/50 transition-colors">
                <div className="flex justify-between items-center h-8 z-10 mb-4">
                  ...
                  <span className="text-2xl font-bold">
                    ...
                    <span className="text-sm text-white/50 font-bold">/ {maxRam.toFixed(0)}GB</span>
                  </span>
                </div>
                <div className="flex-1 relative overflow-hidden bg-[var(--7dtd-bg-panel-dark)] border border-[var(--7dtd-border)] mt-auto z-10">
                  {/* Grid Lines */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <div className="sevendays-panel p-6 flex items-center gap-4 hover:bg-[var(--7dtd-bg-panel-light)] transition-colors border border-[var(--7dtd-border)]">
                <div
                  className={`w-12 h-12 flex items-center justify-center border ${serverStatus === 'Online' ? 'bg-green-500/20 text-green-400 border-green-500/30' : 'bg-red-500/20 text-red-400 border-red-500/30'}`}
                >
                  <span className="material-symbols-outlined text-3xl">power_settings_new</span>
                </div>
                <div>
                  <div className="sevendays-title text-sm text-[var(--7dtd-text-dim)] mb-1">
                    STATUS
                  </div>
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-2.5 h-2.5 rounded-sm ${serverStatus === 'Online' ? 'bg-green-400 animate-pulse' : 'bg-red-500'}`}
                    ></div>
              ...
              <div className="sevendays-panel p-6 flex items-center gap-4 hover:bg-[var(--7dtd-bg-panel-light)] transition-colors border border-[var(--7dtd-border)]">
                ...
                <div>
                  ...
                  <div className="text-xl font-bold">
                    ...
                    <span className="text-[var(--7dtd-text-dim)] text-sm">
                      / {currentServer?.maxPlayers || 8}
                    </span>
                  </div>
                </div>
              </div>
