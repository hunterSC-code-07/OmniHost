    ...
    <div className="absolute inset-0 flex gap-4 p-8 min-h-0 sevendays-ui">
      <div className="flex-[3] flex flex-col min-h-0 min-w-0 sevendays-panel">
        ...
        >
          <div className="p-6 font-mono text-sm text-white/80 flex flex-col min-h-full">
            {logs.length === 0 && (
              <div className="text-white/30 italic mt-4 mb-4 uppercase">
            ...
            {logs.map((log, i) => {
              ...
              return (
                ...
                >
                  {isInfo && !isCommand && (
                    <span className="text-blue-400 font-bold mr-1">INF</span>
                  )}
                  {isWarn && !isCommand && (
                    <span className="text-yellow-400 font-bold mr-1">WRN</span>
                  )}
                  {isError && !isCommand && (
                    <span className="text-red-400 font-bold mr-1">ERR</span>
                  )}
                  {isCommand && <span className="text-white font-bold mr-1">&gt;</span>}
                  <span>
                    {cleanLog
                      .replace(/(INF|WRN|ERR|\[INF\]|\[WRN\]|\[ERR\])/g, '')
                      .replace(/^> /, '')
                      .trim()}
                  </span>
                </div>
        ...
        <form onSubmit={onSubmit} className="p-4 flex gap-4 items-center bg-[var(--7dtd-bg-panel)]">
          ...
          <div className="sevendays-input-container flex-1">
            <input
              ...
              className="sevendays-input w-full px-2 uppercase"
            />
          </div>
          ...
          <button
            ...
            className="sevendays-btn !px-4"
            title="Clear Console"
          >
            <span className="material-symbols-outlined text-[20px]">delete_sweep</span>
          </button>

          <button type="submit" className="sevendays-btn !px-8">
            SEND
          </button>
        </form>
      </div>
      ...
      <div className="flex-[1] sevendays-panel flex flex-col min-h-0">
        <div className="p-4 border-b border-[var(--7dtd-border)] flex justify-between items-center">
          ...
          <div className="px-3 py-1 border border-white/20 bg-white/10 text-white text-xs font-bold">
        ...
        >
          <div className="p-4 flex flex-col min-h-full">
            {onlinePlayers.length === 0 ? (
              <div className="text-center text-white/50 sevendays-title text-sm mt-10">
                NO ONE IS ONLINE
              </div>
            ) : (
              <div className="space-y-2">
                {onlinePlayers.map((playerName, idx) => (
                  <div
                    ...
                    className="flex items-center gap-3 bg-[var(--7dtd-bg-panel)] p-3 border border-[var(--7dtd-border)] cursor-pointer hover:border-white/50 transition-colors"
                  >
                    <div className="w-8 h-8 bg-white/10 border border-white/20 flex items-center justify-center text-white">
                      <span className="material-symbols-outlined text-[20px]">person</span>
                    </div>
                    <span className="sevendays-title text-md text-white truncate">
                      {playerName}
                    </span>
                  </div>
                ))}
