  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <div className="flex bg-[var(--7dtd-bg-panel-dark)] border border-[var(--7dtd-border)] border-b-0">
        ...
        <div className="p-4 text-sm sevendays-title text-[var(--7dtd-text-dim)] w-32 shrink-0">
          SIZE
        </div>
        <div className="p-4 text-sm sevendays-title text-[var(--7dtd-text-dim)] w-48 shrink-0">
          DATE MODIFIED
        </div>
        <div className="p-4 text-sm sevendays-title text-[var(--7dtd-text-dim)] w-24 shrink-0 text-right">
          ACTIONS
        </div>
      </div>
              ...
              files.map((file) => (
                ...
                >
                  <div className="p-4 flex flex-1 items-center gap-3 overflow-hidden min-w-0">
                    <span
                      className={`material-symbols-outlined shrink-0 ${file.isDirectory ? 'text-[var(--7dtd-text-dim)]' : 'text-[var(--7dtd-text-dim)]'}`}
                    >
                      {file.isDirectory ? 'folder' : 'draft'}
                    </span>
                    <span className="text-sm font-bold truncate text-white uppercase">
                      {file.name}
                    </span>
                  </div>
