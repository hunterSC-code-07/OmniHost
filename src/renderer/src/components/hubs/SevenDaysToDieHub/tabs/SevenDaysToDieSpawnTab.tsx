...
export const SevenDaysToDieSpawnTab: React.FC<SevenDaysToDieSpawnTabProps> = ({ serverId }) => {
  ...
  const [targetPlayer, setTargetPlayer] = useState<string>(
    onlinePlayers.length > 0 ? onlinePlayers[0] : ''
  )
      ...
      <div className="flex gap-4 min-h-0 flex-1">
        ...
        <div className="flex-[7] sevendays-panel flex flex-col min-h-0 overflow-hidden border border-[var(--7dtd-border)]">
          ...
          <div className="flex-1 min-h-0 relative bg-[var(--7dtd-bg-panel-dark)]">
            {loading ? (
              <div className="absolute inset-0 flex items-center justify-center text-white/50 sevendays-title">
                LOADING DATA...
              </div>
                ...
                >
                  {rowVirtualizer.getVirtualItems().map((virtualRow) => {
                    ...
                    return (
                      ...
                      >
                        <span
                          className={`text-sm ${isSelected ? 'text-white font-bold' : 'text-white/70'}`}
                        >
                          {item.name}
                        </span>
                        <span className="text-xs text-[var(--7dtd-text-dim)] px-2 py-1 bg-black/40 border border-white/10 font-bold">
                          {item.group}
                        </span>
                      </div>
                    )
        ...
        <div className="flex-[3] sevendays-panel p-6 border border-[var(--7dtd-border)] flex flex-col h-fit">
          <h4 className="sevendays-title text-xl mb-6 pb-2 border-b border-[var(--7dtd-border)]">
            SPAWN SETTINGS
          </h4>
          ...
          <div className="space-y-6">
            ...
            <div className="sevendays-input-row flex-col items-start gap-2">
              <span className="sevendays-input-label">
                SELECTED {actionType === 'item' ? 'ITEM' : 'ENTITY'}
              </span>
