...
export const SatisfactoryAnimatedBackground: React.FC = React.memo(() => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
          ...
          // Orange & Black palette stops for Satisfactory
          vec3 deepBlack = vec3(0.01, 0.005, 0.0);      
          vec3 orange = vec3(0.3, 0.15, 0.0);       
          vec3 brightOrange = vec3(0.6, 0.3, 0.0);      

          float combinedNoise = mix(n1, n2, n3 * 0.4);
          vec3 color = mix(deepBlack, orange, combinedNoise);
          
          float highlightMask = pow(n1 * n2, 4.0);
          color = mix(color, brightOrange, highlightMask * 0.5);
