...
export const SatisfactoryConsoleTab: React.FC = React.memo(() => {
    ...
    <div className="absolute inset-0 flex gap-6 p-6 min-h-0 bg-transparent font-body">
      <div className="flex-1 flex flex-col bg-black/40 backdrop-blur-md rounded-xl overflow-hidden border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.3)] min-h-0 min-w-0">
        ...
        >
          <div className="p-6 font-mono text-sm text-on-surface-variant shadow-inner flex flex-col min-h-full">
            {logs.length === 0 && <div className="text-on-surface-variant/50 italic mt-4 mb-4">Waiting for Satisfactory server output... click Start to boot!</div>}
            {logs.map((log, i) => {
              const cleanLog = log.replace(/[\u001b\u009b][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><]/g, '');
              
              const isInfo = cleanLog.includes('INFO') || cleanLog.includes('Log:');
              const isWarn = cleanLog.includes('WARN') || cleanLog.includes('Warning:');
              const isError = cleanLog.includes('ERROR') || cleanLog.includes('Exception') || cleanLog.includes('Error:');
        ...
        <form onSubmit={onSubmit} className="p-4 bg-transparent border-t border-surface-container-highest flex gap-3 items-center">
          ...
          <button type="submit" className="bg-[#fa9549]/30 hover:bg-[#fa9549]/50 text-[#fa9549] border border-[#fa9549]/50 hover:border-[#fa9549] shadow-[0_0_15px_rgba(250,149,73,0.1)] px-8 py-2.5 rounded-xl font-bold transition-all uppercase tracking-widest text-sm">Send</button>
        </form>
      </div>
      ...
      <div className="w-72 bg-black/20 backdrop-blur-md border border-white/5 shadow-[0_8px_32px_rgba(0,0,0,0.2)] flex flex-col min-h-0 rounded-xl overflow-hidden">
        <div className="p-5 border-b border-surface-container-highest flex justify-between items-center bg-surface-container-highest/20">
          ...
          <div className="bg-[#fa9549]/10 border border-[#fa9549]/30 text-[#fa9549] px-3 py-1 rounded-full text-xs font-bold shadow-[0_0_10px_rgba(250,149,73,0.1)]">{onlinePlayers.length} Online</div>
        </div>
        <OverlayScrollbarsComponent 
          className="flex-1 min-h-0" 
          options={{ scrollbars: { theme: 'os-theme-dark', autoHide: 'leave', autoHideDelay: 200 } }} 
            ...
            {onlinePlayers.length === 0 ? (
              ...
              <div className="space-y-3">
                {onlinePlayers.map((playerName, idx) => (
                  <div key={idx} onClick={() => onPlayerClick(playerName)} className="flex items-center gap-4 bg-surface-container-lowest p-3.5 rounded-xl border border-surface-container-highest shadow-sm cursor-pointer hover:border-[#fa9549]/50 hover:bg-surface-container-lowest/80 transition-colors group">
                    ...
                    <span className="font-label-lg text-label-lg text-on-surface group-hover:text-[#fa9549] transition-colors">{playerName}</span>
                  </div>
                ))}
              </div>
            )}
