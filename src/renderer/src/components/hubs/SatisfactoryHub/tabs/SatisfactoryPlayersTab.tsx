...
export const SatisfactoryPlayersTab: React.FC = () => {
  ...
  useEffect(() => {
    if (activeServerId && window.api.server.getSatisfactoryToken) {
      window.api.server
        .getSatisfactoryToken(activeServerId)
        .then((t) => {
          if (t) setToken(t)
        })
        .catch(console.error)
      ...
      <div className="p-8">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-gray-400 mt-2 text-sm">
              Monitor players currently connected to the server.
            </p>
          </div>
        </div>
        ...
        <div className="bg-darkCard p-6 rounded-xl border border-gray-800 shadow-md mb-6">
          ...
          <p className="text-sm text-gray-400 mb-4">
            Satisfactory 1.0 requires an API Token to track online players. To get one, launch the
            game, go to the <b>Server Manager</b> from the main menu, connect to your server, and
            click on the <b>Console</b> tab. Type{' '}
            <code className="bg-gray-800 px-2 py-1 rounded text-[#fa9549]">
          ...
          {saveStatus === 'error' && (
            <p className="text-red-400 text-sm mt-2">Failed to save token.</p>
          )}
          <p className="text-xs text-[#fa9549]/70 mt-4 italic">
            Note: The official Satisfactory 1.0 API only tracks the number of connected players, not
            their names. Players will appear as "Unknown Pioneer" when connected.
          </p>
        </div>
            ...
            {activePlayers.length === 0 ? (
              <div className="text-center text-gray-500 py-12 flex flex-col items-center justify-center">
                ...
                <p className="text-xs mt-2 text-gray-600">
                  Ensure your server is running, the token is set, and players have joined.
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 pb-4">
                {activePlayers.map((player: any, idx) => {
                  const pName =
                    typeof player === 'string' ? player : player.name || 'Unknown Pioneer'
                  return (
                    <div
                      key={idx}
                      className="bg-[#1a1a1a] border border-gray-800 p-4 rounded-xl flex items-center gap-4 transition-all hover:border-[#fa9549]/50"
                    >
