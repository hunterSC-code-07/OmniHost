...
export const SatisfactoryModsTab: React.FC = () => {
  ...
  const handleInstall = async (modReference: string, versions: any[]) => {
    ...
    try {
      await window.api.server.installSatisfactoryMod(
        activeServerId,
        modReference,
        latestVersion.link
      )
        ...
        >
          <div className="p-4 space-y-4">
            {searchResults.length === 0 && !loading && (
              <div className="text-on-surface-variant/50 italic text-center mt-8">
            ...
            {searchResults.map((mod) => (
              <div
                ...
                className="bg-surface-variant p-4 rounded-lg border border-white/5 flex gap-4 transition-colors hover:border-white/10"
              >
                {mod.logo && (
                  <img
                    src={mod.logo}
                    alt={mod.name}
                    className="w-16 h-16 rounded-md object-cover bg-black/50"
                  />
                )}
                <div className="flex-1 min-w-0 flex flex-col justify-center">
                  <div className="flex justify-between items-start mb-1">
                    <h4 className="font-bold truncate text-lg" title={mod.name}>
                      {mod.name}
                    </h4>
                  </div>
                  <p
                    className="text-sm text-on-surface-variant/70 line-clamp-2 mb-2"
                    title={mod.short_description}
                  >
                    {mod.short_description}
                  </p>
                  <div className="flex justify-between items-center mt-auto">
                    <span className="text-xs text-[#fa9549] font-medium">
                    ...
                    {isModInstalled(mod.mod_reference) ? (
                      <button
                        disabled
                        className="bg-green-500/20 text-green-400 px-4 py-1.5 rounded font-medium cursor-default"
                      >
                        Installed
                      </button>
                    ) : (
