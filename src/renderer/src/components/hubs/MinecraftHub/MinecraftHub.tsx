...
export function MinecraftHub() {
  ...
  const { startServer, stopServer, restartServer, deleteServer, setActiveServerId } =
    useServerStore(
      useShallow((s) => ({
  ...
  const [prevServer, setPrevServer] = useState(currentServer)

  if (currentServer && currentServer !== prevServer) {
    setPrevServer(currentServer)
  }
  const activeServer = currentServer || prevServer
          ...
          <div className="bg-surface/80 backdrop-blur-xl border border-outline-variant/30 shadow-2xl rounded-2xl w-full max-w-md overflow-hidden relative">
            ...
            <div className="p-8 relative z-10">
              ...
              <p className="text-gray-400 mb-8 leading-relaxed">
                Moving existing vanilla or lightly-modded worlds into heavy CurseForge modpacks can
                be complicated and cause corruption. Would you like to create a{' '}
                <span className="text-white font-bold">new server</span> for your modpack instead?
              </p>
          ...
          <div className="flex gap-3 items-center">
            <div className="flex glass-panel rounded-lg overflow-hidden transition-all duration-300 ease-out hover:border-white/30 hover:-translate-y-1 hover:scale-105 hover:shadow-[0_8px_30px_rgba(0,0,0,0.5)]">
              <button
                ...
                className={`relative overflow-hidden group px-4 py-2.5 transition-all flex items-center justify-center ${tunnelStatus === 'Online' ? 'bg-brand/10 text-brand hover:bg-brand/20' : tunnelStatus === 'Starting...' ? 'bg-gray-800/50 text-gray-400 cursor-not-allowed' : 'text-gray-400 hover:text-white'}`}
              >
                <span
            ...
            <button
              ...
              className="relative overflow-hidden group glass-panel px-8 py-2.5 rounded-lg font-bold transition-all duration-300 ease-out hover:-translate-y-1 hover:scale-105 hover:border-brand/60 hover:shadow-[0_8px_32px_rgba(76,175,80,0.4),inset_0_1px_2px_rgba(255,255,255,0.4)] text-brand hover:text-green-300"
            >
              <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/5 to-white/20 opacity-30 group-hover:opacity-60 transition-opacity duration-500 pointer-events-none"></div>
              <div className="absolute -inset-[200%] bg-gradient-to-r from-transparent via-white/20 to-transparent -rotate-45 translate-x-[-100%] group-hover:translate-x-[100%] transition-transform duration-[4000ms] ease-in-out pointer-events-none"></div>
          ...
          <AnimatePresence custom={tabDirection} mode="wait" initial={false}>
            ...
            >
              ...
              {activeTab === 'overview' && (
                <OverviewTab
                  serverVersion={
                    serverMeta ? `${serverMeta.type} ${serverMeta.version}` : 'Loading...'
                  }
              ...
              {activeTab === 'options' && (
                <OptionsTab serverId={activeServer.id} onConfigSaved={fetchServerMeta} />
              )}
              ...
              {/* TAB: SOFTWARE */}
              {activeTab === 'software' && (
                <SoftwareTab serverMeta={serverMeta} onSoftwareChanged={fetchServerMeta} />
              )}
            </motion.div>
          </AnimatePresence>
        </div>
