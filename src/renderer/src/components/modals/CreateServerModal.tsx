...
export function CreateServerModal() {
  ...
  const {
    ...
    steamGuardCode,
    setSteamGuardCode,
    setIsSteamGuardRequired
  } = useModalStore()

  const [newServerName, setNewServerName] = useState('')
  const [newServerType, setNewServerType] = useState(initialCreateServerType || 'Vanilla')
  ...
  const handleCreateServer = async () => {
    ...
    try {
      if (activeGameHub === 'Palworld') {
        setIsCreatingServer(true)
        // @ts-ignore
        const newId = await window.api.server.createServer(
          newServerName,
          'Palworld',
          'Vanilla',
          'Latest'
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50">
      <div
        className={`bg-[#0a0a0a] p-8 rounded-xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.9),inset_0_1px_2px_rgba(255,255,255,0.1)] w-full relative ${newServerType === 'CurseForge Modpack' ? 'max-w-4xl' : 'max-w-md'}`}
      >
