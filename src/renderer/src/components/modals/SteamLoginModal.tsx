...
export function SteamLoginModal({ action, handleCreateServer, onClose }: any) {
  ...
  const { steamCreds, rememberMe, setSteamCreds, setRememberMe, saveCredentials } =
      ...
      <div className="bg-[#0a0a0a] p-8 rounded-xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.9),inset_0_1px_2px_rgba(255,255,255,0.1)] w-full max-w-md relative">
        ...
        <div className="relative z-10">
          ...
          <p className="text-sm text-gray-400 mb-6">
            To download the {activeGameHub} Server files, you must log into SteamCMD. Your
            credentials will be cached locally if you check 'Remember Username for cached logins'.
            Steam Guard codes must be entered when requested.
          </p>
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-bold text-gray-400 mb-1">
                Steam Password{' '}
                <span className="text-xs text-gray-600 font-normal">(Optional if cached)</span>
              </label>
            ...
            {isSteamGuardRequired && (
              <div>
                <label className="block text-sm font-bold text-yellow-500 mb-1 mt-4">
                  Steam Guard Code
                </label>
                <input
                  type="text"
                ...
                <p className="text-xs text-yellow-500/70 mt-1">
                  Check your email for the code. If you approved via mobile push, leave this blank
                  and click Login again.
                </p>
              </div>
            )}
          </div>
