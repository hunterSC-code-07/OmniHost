...
export class SteamWebAPI {
  static async searchWorkshop(
    ...
    try {
      ...
      if (e.response && e.response.status === 429) {
        throw new Error(
          'Steam is rate-limiting your requests. Please wait a few minutes before searching again.'
        )
      }
      throw e
    }
  ...
  static async getModDependencies(modId: string): Promise<string[]> {
    try {
      ...
      if (e.response && e.response.status === 429) {
        throw new Error(
          'Steam is rate-limiting your requests. Please wait a few minutes before checking dependencies again.'
        )
      }
      throw e
    }
  ...
  static async getWorkshopItemDetails(modIds: string[]) {
    try {
      ...
      for (let i = 0; i < modIds.length; i += BATCH_SIZE) {
        ...
        const details = (response.data?.response?.publishedfiledetails || []).filter(
          (mod: any) => mod.result === 1
        )
      ...
      if (e.response && e.response.status === 429) {
        throw new Error(
          'Steam API is rate-limiting your requests. Please wait a few minutes before trying again.'
        )
      }
      throw e
    }
