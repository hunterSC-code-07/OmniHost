    ...
    item.once('done', async (_event, state) => {
      if (state === 'completed') {
        try {
          ...
          let rootFolder = ''
          const hasModInfoAtRoot = zipEntries.some(
            (e) => e.entryName.toLowerCase() === 'modinfo.xml'
          )

          if (!hasModInfoAtRoot) {
            // Find the folder containing ModInfo.xml
            const modInfoEntry = zipEntries.find(
              (e) =>
                e.entryName.toLowerCase().endsWith('/modinfo.xml') ||
                e.entryName.toLowerCase().endsWith('\\modinfo.xml')
            )
            if (modInfoEntry) {
              rootFolder = modInfoEntry.entryName.substring(
                0,
                modInfoEntry.entryName.length - 'modinfo.xml'.length
              )
            }
          }
          ...
          if (rootFolder) {
            ...
            const folderName =
              rootFolder
                .replace(/[/\\]$/, '')
                .split(/[/\\]/)
                .pop() || item.getFilename().replace('.zip', '')
          ...
          if (window) {
            window.webContents.send('mod-download-complete', {
              filename: item.getFilename(),
              serverId: serverIdSnapshot
            })
          }
          ...
          if (window) {
            window.webContents.send('mod-download-error', {
              filename: item.getFilename(),
