...
export class SevenDaysToDieProcessManager {
  ...
  async start() {
    ...
    if (!fs.existsSync(exePath)) {
      this.sendLog(
        `[System] 7 Days to Die Server executable not found at ${exePath}. Did you finish the SteamCMD download?`
      )
      return
    }
    ...
    const args = ['-batchmode', '-nographics', '-dedicated', '-configfile=serverconfig.xml']
  ...
  parseLogLine(line: string) {
    ...
    if (line.includes('joined the game') || line.includes('PlayerSpawnedInWorld')) {
      const match =
        line.match(/GMSG: Player '([^']+)' joined the game/) || line.match(/PlayerName='([^']+)'/)
      if (match) {
    ...
    if (line.includes('left the game') || line.includes('Player disconnected:')) {
      const match =
        line.match(/GMSG: Player '([^']+)' left the game/) || line.match(/PlayerName='([^']+)'/)
      if (match) {
        const playerName = match[1].trim()
