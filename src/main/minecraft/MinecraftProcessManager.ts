...
export class MinecraftProcessManager {
  ...
  async start() {
    ...
    try {
      ...
      this.sendLog(
        `[System] Warning: Failed to download dynamic Java (${err.message}). Falling back to system java.`
      )
    }
    ...
    const batPath = fs.existsSync(runBatPath)
      ? runBatPath
    ...
    if (batPath) {
      ...
      if (parsedArgs) {
        ...
        const filteredArgs = parsedArgs.filter(
          (a) => !a.startsWith('-Xmx') && !a.startsWith('-Xms')
        )
        ...
        this.sendLog(
          `[System] Warning: Could not parse ${batPath === runBatPath ? 'run.bat' : 'start.bat'}, launching via cmd.exe (resource stats may be inaccurate).`
        )
      ...
      if (forgeJar) {
        ...
        this.sendLog(
          `[System Error] server.jar or modloader not found! Please delete and recreate this server.`
        )
        return
      }
    }
