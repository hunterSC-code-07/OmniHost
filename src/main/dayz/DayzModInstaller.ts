...
export class DayzModInstaller {
  ...
  static async importLocalWorkshop(serverId: number, workshopPath: string) {
    try {
      ...
      if (!(await exists(actualWorkshopPath))) {
        throw new Error(
          `Could not find !Workshop folder at ${actualWorkshopPath}. Make sure you selected the correct DayZ directory.`
        )
      }
  ...
  static async installMods(
    ...
    try {
      ...
      await SteamWorkshopDownloader.downloadWorkshopItems(
        serverId,
        appId,
        modIds,
        username,
  ...
  static async installMod(
    ...
    try {
      ...
      // 1. Download via SteamWorkshopDownloader
      await SteamWorkshopDownloader.downloadWorkshopItem(
        serverId,
        appId,
        modId,
        username,
