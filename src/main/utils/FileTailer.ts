  ...
  private pollForFile() {
    ...
    const checkFile = () => {
      ...
      try {
        ...
        files = files.filter(
          (f) => fs.statSync(join(this.options.directory, f)).mtimeMs > this.options.startTime
        )

        if (files.length > 0) {
          files.sort((a, b) => {
            return (
              fs.statSync(join(this.options.directory, b)).mtimeMs -
              fs.statSync(join(this.options.directory, a)).mtimeMs
            )
          })
