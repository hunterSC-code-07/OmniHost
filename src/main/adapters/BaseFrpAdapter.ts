...
export abstract class BaseFrpAdapter {
  ...
  abstract getProxyConfig(localIp: string, portOverride?: number): string
  ...
  async start(ip: string = '34.131.235.17', portOverride?: number) {
    if (this.process) throw new Error('An FRP tunnel is already running')
