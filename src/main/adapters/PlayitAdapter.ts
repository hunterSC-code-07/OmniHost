...
export class PlayitAdapter {
  ...
  async start() {
    ...
    this.process.stderr?.on('data', (data) => this.sendLog(`[Playit Error]: ${data.toString().trim()}`));
