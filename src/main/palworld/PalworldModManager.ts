...
export class PalworldModManager {
  private static getApiKey(): string {
    const FALLBACK_CURSEFORGE_API_KEY =
      '$2a$10$WLjUD.aJlcjuSSdEOByujetqwwhUeTTfS2AsFhIOq31vLq./E1nRO'
          ...
          if (/\/$/.test(entry.fileName)) {
            ...
            zipfile.openReadStream(entry, (err, readStream) => {
              ...
              if (entry.fileName.toLowerCase().endsWith('.pak')) {
                ...
                entry.fileName.toLowerCase().endsWith('main.lua') ||
                entry.fileName.includes('Scripts/')
              ) {
  ...
  static async getInstalledMods(serverId: number) {
    ...
    if (fs.existsSync(scriptsDir)) {
      const scripts = fs
        ...
        .filter((f) => fs.statSync(path.join(scriptsDir, f)).isDirectory() && f !== 'shared')
