...
export class SatisfactoryProcessManager {
  ...
  sendCommand(cmd: string) {
    this.sendLog(
      `[System] Satisfactory commands via console are not natively supported by the standard engine output yet. Command: ${cmd}`
    )
  }
  ...
  async start() {
    ...
    if (!fs.existsSync(exePath)) {
      this.sendLog(
        `[System] Satisfactory Server executable not found at ${exePath}. Did you finish the SteamCMD download?`
      )
      return
    }
