...
export class SonsOfTheForestProcessManager {
  ...
  sendCommand(cmd: string) {
    if (!this.process || !this.process.stdin) {
      this.sendLog(
        `[System] Process not running or stdin not available, cannot send command: ${cmd}`
      )
      return
    }
  ...
  async start() {
    ...
    if (!fs.existsSync(exePath)) {
      this.sendLog(
        `[System] Sons of the Forest Server executable not found at ${exePath}. Did you finish the SteamCMD download?`
      )
      return
    }
    ...
    const args = ['-batchmode', '-nographics', '-dedicated', '-userdatapath', this.serverDir]
    ...
    this.process.stderr?.on('data', (data) =>
      processLogData(data, 'SonsOfTheForest Error', stderrBuffer)
    )
  ...
  parseLogLine(line: string) {
    if (line.includes('joined')) {
      const match =
        line.match(/Player\s+([^ ]+)\s+joined/i) || line.match(/joined the game:\s*(.+)/i)
      if (match) {
    ...
    if (line.includes('left') || line.includes('disconnected')) {
      const match =
        line.match(/Player\s+([^ ]+)\s+(left|disconnected)/i) ||
        line.match(/(left|disconnected):\s*(.+)/i)
      if (match) {
        const playerName = (match[1] || match[2]).trim()
