...
export function registerPalworldIpc() {
  ...
  ipcMain.handle('set-palworld-config', async (_, serverId: number, config: Record<string, string>) => {
    return await PalworldConfigManager.setConfig(serverId, config);
  });

  ipcMain.handle('search-palworld-mods', async (_, query: string, categoryId?: number, index?: number, pageSize?: number) => {
    return await PalworldModManager.searchMods(query, categoryId, index, pageSize);
  });

  ipcMain.handle('install-palworld-mod', async (_, serverId: number, modId: number, fileId: number) => {
  ...
  ipcMain.handle('uninstall-palworld-mod', async (_, serverId: number, modType: string, modName: string) => {
