        ...
        for (const entry of entries) {
          if (entry.isDirectory()) {
            ...
            if (fs.existsSync(modInfoPath)) {
              ...
              const nameMatch =
                content.match(/<Name[^>]*value="([^"]+)"/i) ||
                content.match(/<Mod[^>]*>\s*<Name[^>]*>([^<]+)<\/Name>/i)
              const versionMatch =
                content.match(/<Version[^>]*value="([^"]+)"/i) ||
                content.match(/<Version[^>]*>([^<]+)<\/Version>/i)
              const authorMatch =
                content.match(/<Author[^>]*value="([^"]+)"/i) ||
                content.match(/<Author[^>]*>([^<]+)<\/Author>/i)
              const descMatch =
                content.match(/<Description[^>]*value="([^"]+)"/i) ||
                content.match(/<Description[^>]*>([^<]+)<\/Description>/i)

              const dependencies: string[] = []
              const dependsOnRegex = /<DependsOn[^>]*value="([^"]+)"/gi
              let depMatch
              while ((depMatch = dependsOnRegex.exec(content)) !== null) {
                dependencies.push(depMatch[1])
              }
              // Also check alternative format <DependsOn>ModName</DependsOn>
              const altDependsOnRegex = /<DependsOn[^>]*>([^<]+)<\/DependsOn>/gi
          ...
          (dep: string) =>
            !mods.some(
              (m) =>
                ...
                m.folderName.toLowerCase() === dep.toLowerCase()
            )
        )
      }
    ...
    async (_event, serverId: number, folderName: string, enabled: boolean) => {
      try {
        ...
        if (!fs.existsSync(disabledModsDir))
          await fs.promises.mkdir(disabledModsDir, { recursive: true })
