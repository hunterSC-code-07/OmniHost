...
import { steamCacheStorage } from './SteamCacheStorage'

export class SteamCache {
  static getCacheDir(appId: number) {
    return join(steamCacheStorage.getPath(), appId.toString())
  }
  ...
  static async isCached(appId: number): Promise<boolean> {
    ...
    const gameConfig = Object.values(steamConfigs).find((g) => g.appId === appId)
    if (
      gameConfig &&
      gameConfig.executable &&
      fs.existsSync(join(cacheDir, gameConfig.executable))
    ) {
      return true
    }
