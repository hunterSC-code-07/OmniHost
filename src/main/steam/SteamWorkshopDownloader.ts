            ...
            if (totalSize > 0 && fs.existsSync(targetDir)) {
              ...
              const checkSize = async (dir: string) => {
                ...
                try {
                  ...
                  /* ignore read errors during active download */
                }
              }
              ...
              SteamCMDSetup.sendLog(
                serverId,
                0,
                `[MOD:${modId}] Downloading Mod Files (Indeterminate)...`
              )
            }
          }, 1000)
            ...
            if (output.trim()) {
              ...
              if (
                ...
                (lowerOutput.includes('invalid password') ||
                  lowerOutput.includes('account login denied') ||
                  lowerOutput.includes('password required'))
              ) {
              ...
              if (
                lowerOutput.includes('steam guard') ||
                lowerOutput.includes('two-factor') ||
                lowerOutput.includes('enter the current code')
              ) {
