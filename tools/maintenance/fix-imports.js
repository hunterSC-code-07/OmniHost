...
filesToFix.forEach((file) => {
  if (fs.existsSync(file)) {
    ...
    content = content.replace(/import\s*{\s*([^,]+),\s*app\s*,\s*([^}]+)\s*}\s*from\s*'electron'/g, "import { $1, $2 } from 'electron'");
    content = content.replace(/import\s*{\s*app\s*,\s*([^}]+)\s*}\s*from\s*'electron'/g, "import { $1 } from 'electron'");
